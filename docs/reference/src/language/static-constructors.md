...
四个触发点对依赖包里的类型同样成立，且与使用顺序无关（2026-09-15 前「跨包第一次使用是调静态方法」时静态
构造器可能不执行，见 [实现](../../../internals/src/runtime/static-ctor-init.md#登记点必须早于第一次使用)）。
...
[静态构造函数的按类型初始化](../../../internals/src/runtime/static-ctor-init.md)。
