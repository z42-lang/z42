...
> ⚠️ **格式说明符不支持，写了报 `E0492`。** `$"{x:X2}"` / `$"{x:F2}"` 这类 C# 写法在 z42 里
> 是错误：
>
> ```z42
> int r = 255;
> Console.WriteLine($"#{r:X2}");   // E0492: unexpected `:` after the expression in this interpolation hole
> ```
>
...
> 坐标）。要修得给 Parser 穿透真实 file + 偏移，独立一件事。
