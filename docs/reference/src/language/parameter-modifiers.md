...
> 对齐：2026-09-21 ｜ 实测基准：`./.z42/z42 run`
...
> | `void ByValue(int x)`（形参无修饰符）却调用成 `ByValue(ref v)` | 编译报错 | 编译通过，**方法里的写入传回了调用方** |
>
> 最后两条最容易咬人，而且是**相反的两个方向**：
> - 漏写 `ref`（形参有、调用点无）不报错，也没有任何运行期迹象，`v` 就是没变；
> - 多写 `ref`（形参无、调用点有）同样不报错，而 `v` **被改了**——
>   按引用与否实际由**调用点**决定，形参上的修饰符对调用者的可见效果没有约束力。
>   这意味着**只看函数声明判断不出参数会不会被改**，自由函数与类方法皆然。
>
> **在这些检查补上之前，调用点请自己写全修饰符、且只在形参声明了修饰符时写**，
> 并把 `out` / `in` 当成纯粹的**可读性标注**。
>
...
| 规则 | 期望行为 | 现状 |
|---|---|---|
| 调用点与形参的修饰符必须一致 | 形参无修饰符时 `Foo(ref v)` 报错；形参有 `ref` 时 `Foo(v)` 报错 | 未实现——两个方向都编译通过，**按引用与否由调用点决定** |
