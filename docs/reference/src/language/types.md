...
> **跨包**：类方法与自由函数的形参/返回值标记、**以及字段与属性的标记**都**跟过包边界**，
> 义务照常成立；只有**接口成员**的标记**不跟**（接口在包格式里走另一套不带标记位的结构），
> 导入的接口成员一律视为未标。
> **转换位**：`(int)x` / `(long)x` —— 目标是**值类型**时**算解引用**，义务照常成立
> （不检查就报错）；目标是**引用类型**（`(string)x` / `(Box)x`）时不算，null 合法通过得 null。
> 这条界线与运行期一致：前者遇 null 抛 `NullReferenceException`，后者放行。
> `x as T` 不算解引用（失败返 null）。
