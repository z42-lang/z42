...
```z42
string result = maybeNull ?? "default";   // 左侧为 null 时取右侧
int? len      = maybeNull?.Length;        // 左侧为 null 时整体为 null，不解引用
```

两者按**运行期**的 null 值工作。注意 z42 的 `?` 类型标记会被类型解析擦除、不做静态空安全
检查，见[基本类型与字面量](types.md)的「可空标记」一节。
