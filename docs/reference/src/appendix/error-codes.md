...
| E0480 | 使用了已移除的 `?.`（空条件成员访问）。它把「可能为 null」静默向下传播，崩溃现场离病因很远。诊断给迁移写法，并**按普通 `.` 继续解析**以免级联错（同 E0471 对 `out`/`in` 的手法）。⚠️ 顺带修掉一个真 bug：旧脱糖把接收者**绑定两次** ⇒ `F()?.X` **调用 `F` 两次**（原注释自述已知而未修） | ✅ `ExprParser.z42`（字面量发码） | `var v = n?.value;` |
