...
2. **优化本身的时间/内存开销要有度**:
   ...
   - 运行时分析(加载期/JIT 期)不能拖垮加载与执行;能编译期算好的静态分析就别留到运行时反复算(见 [self-hosting](../compiler/self-hosting.md) 的 REGT 先例:reg_types 编译期算好,运行时只消费)。
...
> 运行时 per-context arena、诊断防线详见 [逃逸分析与栈上分配](escape-analysis.md)。
