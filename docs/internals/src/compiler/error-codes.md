...
| `E10xx` | 调用实参绑定 | `E1001` / `E1002` 参数绑定错误；`E1005` 缺必填实参、`E1006` 实参过多（见下） |
...
> 不同可见 ns → `E0456`。
>
> **`E1005` / `E1006`（普通调用实参个数，add-call-arity-diagnostics）**：非 `params` 调用里，
> 实参多于形参 → `E1006`；少传且缺位形参**没有默认值** → `E1005`（同包看声明的默认值，跨包看导入签名的
> `$Default` / caller 宏；消息点名缺的形参——同包给名字、跨包给序号，导入签名不带形参名）。覆盖全部调用
> 形态：实例 / 类·基元关键字·ns 限定静态 / 同类无限定静态 / 自由函数 / `ns.func()` / 局部函数 / 跨包构造器
> （本地构造器由 `E0426` 管）。**静态形式调实例方法**（`Int32.ToString(n)`，接收者作首参）只接受
> 「形参数 + 1」个实参，否则同样报这两个码。此前这些形态**全部静默**：多传直接发码，少传原样发出不足的
> 实参（同包）或**补零值**（跨包——补完个数「对了」，连运行期签名判定都拦不住）。
>
...
> 据此判「输入没写完、需续读」。机制见 [REPL 输入完整性判定](../../../book/src/toolchain/repl-input-completeness.md)。
