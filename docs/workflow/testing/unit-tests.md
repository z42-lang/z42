...
覆盖：lexer / parser / type-check / IR-gen 经「z42c 自编译自身 + 重建产物逐字节一致」端到端验证，外加编译器源码里的 `[Test]`-注解 units。这条是 GREEN gate 的编译器关。机制见 [`bootstrap.md`](bootstrap.md) 与 [`docs/internals/src/compiler/self-hosting.md`](../../internals/src/compiler/self-hosting.md)。
