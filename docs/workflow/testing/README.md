...
CI 全绿门禁（`cargo build`（z42vm）+ `./xtask test`（内部串联 e2e / cross-zpkg / stdlib / compiler））的定义见 [`../ci.md`](../ci.md)；规则在 [`../../agent/rules/workflow.md`](../../agent/rules/workflow.md) 阶段 8。
...
|--------|---------------------------|-----------|
| `test-host`(×4 OS) | 始终运行 | e2e goldens（interp）+ cross-zpkg（仅 linux-x64）+ lines（行数硬上限棘轮，随 `test all` 跑）|
