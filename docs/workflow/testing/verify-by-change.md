...
|------|------------------------|------------------|------------|
| **编译器 `src/compiler/`** | `xtask test changed`（→ compiler + vm） | 触及 lexer/parser/codegen/格式 writer，或 z42c 源用了新写法 → `xtask test bootstrap` | `compiler-checks`(自举不动点+vscode)、`verify-selfhost`、`test-host`(e2e goldens)、`cross-zpkg` |
| **标准库 `src/libraries/`（加 API / 改实现）** | `xtask test changed`（→ lib <lib> + vm）或 `xtask test stdlib <lib> --filter=K` | — | `stdlib-interp`+`stdlib-jit`(分片)、`test-host`(e2e goldens)、`cross-zpkg` |
...
| **VM `src/runtime/`（Rust）** | `cargo test --manifest-path src/runtime/Cargo.toml` + `xtask test e2e` | — | `test-host`(e2e goldens)、`vm-jit`(4 shard)、`stdlib-interp`+`stdlib-jit`、`cross-zpkg`、`verify-features` |
| **仅测试用例 `src/tests/`** | `xtask test e2e` | — | `test-host`(e2e goldens)、`cross-zpkg`。⚠️ `vm-jit`/`stdlib-*` 不跑（测试用例不影响 JIT 行为/stdlib 单元） |
| **xtask 源 `scripts/`** | `z42 publish scripts/xtask.z42.toml` 重建 → 随便跑条命令冒烟 | changed 映射对 `scripts/xtask*` = **full**（完整 gate） | ci-bootstrap step 2（种子编 xtask 源）；`cross-zpkg`(tests filter) |
