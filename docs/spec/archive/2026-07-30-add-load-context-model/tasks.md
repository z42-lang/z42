# Tasks: 加载上下文模型（AssemblyLoadContext / ALC 地基）
...
## 阶段 1: 运行时上下文模型（Rust）
- [ ] 1.1 `metadata/context.rs`（NEW）：`ContextId` / `AssemblyLoadContext`（name, is_collectible, arena）/ `ContextRegistry`（root=ContextId(0) + collectible 表 + create_collectible）
...
- [ ] 2.2 `corelib/assemblyloadcontext.rs`（NEW）：`__lctx_default / __lctx_create_collectible / __lctx_load / __lctx_name / __lctx_is_collectible / __lctx_assemblies / __lctx_unload`
- [ ] 2.3 `corelib/assemblyloadcontext.rs`：`__asm_name / __asm_is_collectible / __asm_loadcontext / __asm_get_types`
- [ ] 2.4 `corelib/assemblyloadcontext.rs`：`__type_is_collectible / __type_assembly`
- [ ] 2.5 `corelib/mod.rs`：`pub mod loadcontext;` + BUILTINS 表追加注册（表尾，勿插中间）

## 阶段 3: stdlib z42 类
- [ ] 3.1 `Runtime/AssemblyLoadContext.z42`（NEW）：`Std.Runtime.AssemblyLoadContext`（Default / Name / IsCollectible / CreateCollectible / Load / GetAssemblies / Unload）
- [ ] 3.2 `Reflection/Assembly.z42`（NEW）：`Std.Reflection.Assembly`（Name / IsCollectible / AssemblyLoadContext / GetTypes）
...
## 阶段 4: 测试
- [ ] 4.1 `corelib/assemblyloadcontext_tests.rs`（NEW）：注册表 / root vs collectible / IsCollectible / Unload 抛异常
