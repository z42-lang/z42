# Proposal: 加载上下文模型（LoadContext / ALC 地基）
...
只做**地基**：引入 dotnet `AssemblyLoadContext` 对标的 `LoadContext` 模型，把 zpkg 载入
...
- **运行时 `LoadContext` 模型**：`root`（core/stdlib/主程序所在，永驻不可回收）+ 可创建的
...
- **`Std.Runtime.LoadContext`（新 z42 类）**：`Default` / `Name` / `IsCollectible` /
...
- **`Std.Reflection.Assembly`（新 z42 类）**：zpkg 的运行时反射投影，native 句柄背书（仿 `Type`）。
  `Name` / `IsCollectible` / `LoadContext` / `GetTypes()`。
- **`Std.Type` 追加两成员**：`IsCollectible`（== `this.Assembly.IsCollectible`）+ `Assembly`
  （定义此类型的 assembly，镜像 .NET `Type.Assembly`）。
- **Rust 侧**：新 `corelib/loadcontext.rs`（`__lctx_*` / `__asm_*` / `__type_is_collectible` /
  `__type_assembly` builtins）+ `VmCore` 上下文注册表 + `TypeDesc` 的 context/assembly 回指 +
  `Value` 的 native handle 变体（LoadContext / Assembly 句柄）。

## Scope（允许改动的文件）

| 文件路径 | 变更类型 | 说明 |
|---------|---------|------|
| `src/libraries/z42.core/src/Runtime/LoadContext.z42` | NEW | `Std.Runtime.LoadContext` 类 |
...
| `src/runtime/src/corelib/loadcontext.rs` | NEW | LoadContext / Assembly / Type-collectible builtins |
| `src/runtime/src/corelib/mod.rs` | MODIFY | `pub mod loadcontext;` + BUILTINS 表注册新 builtins |
| `src/runtime/src/metadata/context.rs` | NEW | `LoadContext` / `ContextId` / `ContextRegistry` 运行时模型 |
...
| `docs/book/src/runtime/load-context.md` | NEW | book 机制页：LoadContext 模型 + root/collectible + Assembly 反射（知识上浮） |
...
| `src/runtime/src/corelib/loadcontext_tests.rs` | NEW | Rust 单测：注册表 root vs collectible / load_into / assembly_types 排序 / 可回收性传播 |
...
- **动 `Std.Runtime.Runtime.LoadZpkg` / `CallStatic` stub**（FC3）：保留不动，`LoadContext.Load`
