...
- [x] 1.1 `IrInline`：`_writtenParams` 判 callee 形参是否被写；只读形参 body 中直接 remap 到 `arg[p]`，
...
- [x] 1.3 内联单测更新/新增：直代入后 IR 形态（无 param copy）、const 实参内联折成常量、独立性
- [x] 1.4 `xtask test` 全绿 + self-host 不动点。**注（D7）**：改内联变换（param-copy→直代入）当次
