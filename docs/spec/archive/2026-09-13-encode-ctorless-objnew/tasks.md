...
- [x] 4. regen `src/tests/zbc-format/*/source.zbc`（6 个）—— **需新格式工具链**
- [x] 5. z42c golden hex 单测 `zbc_tests.z42` 内嵌 hex 重截 —— **需新格式工具链**
...
- [x] 9. regen `src/tests/zpkg-format/*/source.zpkg`（4 个，手工）—— **需新格式工具链**
...
- [x] **退回对照**：`_skew` 在未改的编译器上必须 FAIL（打印 `constructed 0`）
- [x] `xtask test e2e --dir cross-zpkg` 两后端（含 `--mode jit`）
- [x] 对账：装配点 census——3898 站点置位 97.2%，未置位 106 个全是 13 个真零构造器类（零误判）

## ✅ 已解除的阻塞：两代自举 prelude 回归（PR #631）
...
- [x] 推 PR 第一轮 → 取 `compile-toolchain` 的 `toolchain-macos-15` artifact
- [x] overlay 成本地种子（保留自己的 cargo `z42vm`）+ `Z42_PORTABLE_VM`
- [x] `xtask build compiler && xtask build stdlib && xtask build test`
- [x] `cargo test`（含 `zbc_compat` / `format_fixture_versions` / `lazy_loader`）
- [x] `xtask test` 完整 GREEN + 自举字节不动点 gen1 == gen2
- [x] 冷构建复验（编译器类改动，规范 §3.1）—— 由 CI 冷启动的 `compile-toolchain` / `test-host` 承担（两代自举即全冷路径）
- [x] `xtask test bootstrap` —— 格式 bump 下「上一版 nightly 编当前源」由 CI 两代自举实跑验证（gen1 即此检查）

## 收尾

- [x] book 机制页 `runtime/missing-symbol-resolution.md` 改写构造器那一节
- [x] 归档 `changes/` → `archive/`（随 PR 同一提交）
- [x] memory `dep-version-skew-program`「② 的墙」改写为已闭合
