...
🔴 **2026-09-25 归档复核更正：上面这条结论的边界写宽了。**
实测覆盖的只有**非泛型**字段（`value_field_zero` 至今也只有非泛型）。**泛型型参字段没有零值** ——
`class GBox<T> { public T V; }` 的 `GBox<int>().V` 读出 `Null` 而不是 `0`，还带一条
interp / jit 分叉。所以「值类型的存储槽永不含 `Value::Null`」这条不变式**今天并未全域成立**。
详见 tasks.md 文末「已知未堵的洞」。根因是**布局按定义算**（型参名落 `GcRef` 槽），
不在本节列的那 6 个分配点上，移交 `complete-generic-instantiation` 线。

⭐ 教训：起草阶段的「grep 出 N 处站点」不等于「这 N 处都需要改」——**先写一个探针实跑**，
比读站点列表快也准。我这条判断是在一棵落后的主树上读代码得出的。

⭐⭐ 配套教训（复核时才看清）：探针推翻了「站点列表」，但**探针自己的覆盖面也要数**。
只造了非泛型字段就写下「实测已经是对的」，等于把一句**有边界**的实测结论记成了**无边界**的结论。
