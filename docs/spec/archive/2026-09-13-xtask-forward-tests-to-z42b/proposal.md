# Proposal: xtask 的 stdlib 测试路径转发 z42b（覆盖面与性能双持平，价值在统一）
...
### ② 覆盖面与性能**双双持平**（**三次订正后的最终数据**）

同机、同并行度、背靠背测量：

| 配置（`xtask test stdlib --no-build --jobs 4`） | 通过用例 | 单元 | 墙钟 |
|---|---|---|---|
| 遗留路径 | 3240 | 332 | 195.17s |
| **转发路径** | **3240** | **332** | **193.09s** |
| 对照：只回退 `NativeParkGuard`（#598/#600） | 3240 | 332 | 193.49s |

**覆盖面逐个用例相同** —— 3240 通过 / 332 单元 / 2 跳过，两边一字不差。**墙钟差 2s，在噪声内。**

> ⚠️ **本条此前错过三次，错因各不相同，一并记在这里当反面教材：**
> 1. 「快 5.6×」——**冷**跑的 xtask（含整个构建波）比**热**跑的 z42b，条件没对齐；
> 2. 「快 2.76×」—— 遗留路径 `jobs=1` 比 z42b **8 路并行**，并行度没对齐；
> 3. 「慢 2.5×」—— 依据是 gate 读数（stdlib stage 1m15s → 3m07s），但那台机器上**并行跑着
>    别的会话的构建**（load average 9~13，另有两个 worktree 的 z42vm 各占一核）。同一时段
>    **遗留路径的 gate 读数也是 3m15s、构建波从 37s 涨到 1m43s** —— 两者同步三倍，
>    代码侧找不到对应变更。**环境没控制。**
>
> 共同教训：**性能结论必须同机、同并行度、背靠背、且确认机器空闲**；
> 任一条不满足，得到的数字就不能拿来做取舍。

⇒ 转发**没有性能代价**，理由完全落在统一性上。
