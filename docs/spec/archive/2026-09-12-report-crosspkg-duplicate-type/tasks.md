...
> 状态：🟢 已完成 | 创建：2026-09-12 | 完成：2026-09-12 | 类型：lang（接线 E0601 + 新增 W0606）
...
**本地遮蔽的实测（决定了 W0606 的文案）**：本包也声明同 FQN 时，`bare` 与 `qualified` **都**绑到
...
**本地遮蔽 = W0606 warning**（对标 C# CS0436，User 裁决）：本地恒赢是既定规则、不是猜。

## 🔴 顺带修的一条：z42c 从不打印 warning

实现到一半发现 W0606 **不会出现在终端**——`Main.z42` 的两个诊断打印点都被 `ErrorCount > 0` 罩着，
...
- [x] **退回对照（接口那条单独做）**：撤掉 `ShadowedImportMsg` 的 `InterfacesByFqn` 分支 →
      ...
      而 W0606 一开始只查 `ClassesByFqn` ⇒ 「本地接口遮蔽导入接口」会单独哑掉
- [x] e2e 负例 fixture `dup_fqn_crosspkg` PASS；cross-zpkg 26/26
