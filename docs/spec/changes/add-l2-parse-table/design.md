...
## D2-6（**已裁决：甲**，User 2026-09-26）：`pattern_match` 的边界

模式引擎入口**实测 5 处**（`_patP` 的全部调用点，grep 坐实）：

| # | 位置 | 形态 |
|---|---|---|
| 1 | `ExprParser.z42:95` | switch **表达式** 的 arm 模式 |
| 2 | `ExprParser.z42:154` | `is` 的 `@` 绑定（`x is name @ P`）|
| 3 | `ExprParser.z42:162-163` | `is` 的字面量/`-`/`_`/`{`/关系算子引导 + `\|` or-链 |
| 4 | `ExprParser.z42:172-178` | `is` 的类型引导结构化（`(`/`{`）与纯类型名 or-链 |
| 5 | `StmtParser.z42:76` / `:400` | switch **语句** 的 `case` 模式 / 解构声明 |

**不在其内**：`ExprParser.z42:181-183` 的老 `is` —— `x is T` / `x is T v`，
源码注释明写「**byte-identical，不入模式引擎**」⇒ 它是**类型测试**，归 `oop` 那一族的基础能力。

**裁决 = 甲**：门挂在上述 5 个入口，`x is T` / `x is T v` 保留。

🔴 **必须写进文档的后果**：`pattern_match = false` 时 switch **语句**本身仍归 `control_flow`
（还能解析），但它的**每个 `case` 都会报 E0301** ⇒ 实际不可用。不写明的话，下一个人会以为
「关掉 pattern_match 后 switch 还能当常量跳转表用」。
（否掉的乙案是「常量模式豁免」：那需要把门从**入口判定**改成**出口判定**（解析出模式后看是不是
`ConstPattern`），与「表项挂 feature」的形态不一致，还要为「常量模式算不算模式」另立定义。）
