...
- [ ] 4.3 S3：不传递不递归，只提升一层
- [ ] 4.4 **`[build] emit_generated`**（User 已裁决：走 toml 不走 CLI flag，没配置即默认值）：
      bool、**默认 false**；`ManifestLoader._parseBuild` 工程级(:216) + 工作区级(:167) 两处，
      沿用 `incremental` 的 bool-带默认形状。写到
      `<output_dir>/generated/<pkg>/__gen$<Name>$augment.z42`（从既有 output_dir 派生，不新立路径根）。
- [ ] 4.5 🔴 **落盘目录必须在 `[sources] include` 扫描范围之外**——否则下次构建把生成的 partial
      碎片当用户源码再编一遍 ⇒ 同名成员两份碎片 ⇒ 直接踩 1.C 的**静默覆盖**（用户方法被吃掉 +
      假类型错）。默认落 `artifacts/` 天然在范围外；用户把 `output_dir` 指进 `src/` 时**报错拒绝**。
...
- [ ] 5.5 `emit_generated` 的落盘目录落在源码扫描范围内 → **报错**（见 4.5；这是个会静默吃掉
      用户方法的配置，不能只 warn）
...
- [ ] 6.6 `emit_generated` 两态各一条：默认（不写盘、产物字节与开之前一致）/ 开启（文件真出现、
      内容可读、且**再编一次仍字节不动点**——证明落盘不参与编译输入）
- [ ] 6.7 jit 双验 `xtask test stdlib --mode jit`
