...
| PR7-wire | **接线 `z42c build --fix` 就地重写源**（契约已随 nightly cdc16e2 进种子）：`DiagSinkImpl : DiagSink, FixSink` 收集编辑 → `AnalyzerDriver.Run(...,fix)` 走完 CU 后按 `TextEdit.At.Start` 降序就地重写 `At.File`（`String.Substring` splice，因 Span.Start 是 Lexer 字符索引非真字节）；driver `--fix` flag→`cin.Fix`→`_runAnalyzers` 调 6-参 `Run`；semantics 加 z42.io 直接依赖（File）；可修复 demo analyzer（Z9010，FixSink 下转）+ 跨包 golden（fixture 携修复就地重写真实 consumer 文件 + Fix=off 不改）。零格式-bump | 否 | ✅ 收官 |
