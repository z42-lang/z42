...
## 进度概览
- [x] P0 前置：worktree 供种 + baseline GREEN
- [x] P1 编译器前端：parser `name!(args)` + 位置放宽 + 改名
- [x] P2 编译器语义：`available!` 绑定、符号解析、dispatch key 生成、诊断
...
## P0 前置
- [x] 0.1 worktree 供种（nightly SDK == origin/main `54c8a1df`，格式/API 双零 skew）
- [x] 0.2 baseline `xtask test` 全绿（**改动前**跑一次，区分 pre-existing 红）
  - ⚠️ GREEN 前 `rm -rf /tmp/z42c-e2e-*`（stale fixture 假失败）

## P1 编译器前端（syntax）
- [x] 1.1 `ExprParser.z42`：`ident '!' '(' ...')'` 支持**带参**形态。
...
- [x] 1.2 保持零新 token（`Bang` / `LParen` / `RParen` 全是既有）
- [x] 1.3 `CallerMacro.z42` → `MacroRegistry.z42`（A4）；更新 4 个引用点
      （`ClassDescBuilder` / `DeclBinder` / `OverloadBinder` / `ExprTyper`）

## P2 编译器语义
- [x] 2.1 `MacroRegistry.KindOf`：白名单加 `available`
- [x] 2.2 宏元数据分类：区分「参数默认值位宏」（caller 类，位置受限）与「表达式位宏」
      （`available`）。`ExprTyper._bindIdent` 的 E0450 拦截改为**按 kind 判定**，
      不再一刀切（现状：任何宏落到 `_bindIdent` = E0450）
- [x] 2.3 `available!(X)` 的参数解析：X 是符号引用（`IdentExpr` / `MemberExpr` 链），
      解析到类型或唯一方法。**不存在 → E0401**（复用）
- [x] 2.4 重载消歧：解析到多个方法 → 新诊断码 A「ambiguous」（A1）
- [x] 2.5 参数形态非法（字面量 / 任意表达式）→ 新诊断码 B
- [x] 2.6 dispatch key 生成（design D2）：
      - 方法 → `"m:" + <与运行期 dispatch 同源的 mangled 全签名键>`
      - 类型 → `"t:" + FQN`
      **关键**：键必须与 VM 实际 dispatch 用的键同源，否则「探测说有、调用却炸」
- [x] 2.7 `DiagnosticCodes.z42`：新增码 A/B（按现状顺延号段）

## P3 编译器发射
- [x] 3.1 emit：`Intern(key)` → `ConstStrInstr` → `BuiltinInstr(dst:Bool, "__sym_available", [key], 1)`
      （形状照抄 `TypeOpEmitter._emitBox` 的 `__box_prim`）
- [x] 3.2 结果类型 `bool`，可直接进 `if` 条件 / 局部初始化

## P4 VM
- [x] 4.1 `builtin_table.rs`：**表尾追加** `("__sym_available", …)`（下标即 BuiltinId，只可追加）
- [x] 4.2 builtin 实现：按 key 判定可解析性。**正常路径永不执行**（已被 P4.3 折掉）→
      debug profile 下走到即告警/panic「fold pass did not run」（design Implementation Notes）
- [x] 4.3 `fold_availability` 加载期 pass（新文件，`metadata/loader/` 下）：
  - [x] 4.3a 判定：DEPS `namespaces` 否定判定 → 定向 force-load 单个 dep → 精确判定（A2/D4）
  - [~] 4.3b 环检测：**实现后判定为不需要**——`probe_declared` 只做「加载一个 artifact 并查表」，
        **不递归**，故不存在环。DRAFT 阶段把它列为风险是高估了（当时以为 probe 会走 lazy loader
        的递归解析）。已按实际去掉，不留空实现假装做了。
  - [x] 4.3c 折叠：`Builtin(__sym_available)` → `ConstBool`
  - [x] 4.3d 剪枝：`BrCond`(常量) → `Br`；BFS 移不可达块。
        **照抄 `IrDeadBranch.z42` 算法形状，不跨语言复用代码**（D8）。
        约束：**有异常表的函数只折不删块**（CFG 不含异常隐式边）
- [~] 4.4 fast path：实现为 `module_has_probe()` **进 pass 时扫一遍**，而非 decode 时置 flag。
      效果等价（无探测则立即返回），但代价是每次加载多一次 O(指令数) 扫描。
      decode 期置 flag 更省，属可选优化，**未做**——先不为没量过的开销加复杂度。
- [x] 4.5 接入装配链 `artifact.rs`：必须在 `build_block_indices` **之前**（或之后重建派生侧表：
      `block_index` / `branch_targets` / `site_index`）
- [x] 4.6 **不做**寄存器重编号（`max_reg` 只增不减；避免动 `reg_types` 与 JIT 2C 提升资格分析）
- [x] 4.7 debug-only 统计量（A3）：折叠数 / 移除块数，经 `Z42_LOG` trace 或测试查询暴露

## P5 测试
- [x] 5.1 **skew 脚手架**（今天零覆盖，可能是本 change 最大工作量）：
      同一依赖包两份源（`dep-v1/` 缺符号 / `dep-v2/` 有），各建 zpkg，用例按需切换 `libs/`。
      需 test-runner 支持「按用例切换依赖产物」——**IMPL 前实测评估，若过大则先出最小形态**
- [x] 5.2 编译器单测：解析 / 位置放宽 / E0401 / 码 A / 码 B
- [~] 5.3 Rust 单测：只覆盖了**纯函数**（`namespace_of` / `claims_namespace` / `stats.is_noop`，4 个）。
      折叠 + 剪枝 + 异常表约束**没有单测**——`Module`/`Function` 无 `Default`，合成一个完整模块
      成本过高。改由 e2e 覆盖且**实测确认走到了分支**：
      `available_exception_table` 跑出 `funcs_kept_for_exceptions=1 / blocks_removed=0`，
      `available_present|skew` 跑出 `blocks_removed=1`。
...
| P4.3a force-load 引发启动放大 | ns 否定判定先行；只加载认领该 ns 的单个文件。**环不存在**：probe 不递归（见 4.3b） |
