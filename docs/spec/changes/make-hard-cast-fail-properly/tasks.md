...
> 状态：🟡 实施中（运行期半部完成；引用类型那支见末尾拆分说明） | 创建：2026-09-22
...
## 阶段 1
- [x] 1.1 `Exceptions/InvalidCastException.z42`（与既有 18 个同形）
- [x] 1.2 ~~`CompilerFingerprint++`~~ —— **不需要**。指纹管的是「同一份源码 + 同样格式，编出的字节却变了」
      （codegen / 优化 / typecheck / lowering 改动）；本变更加的是**源文件** + 运行期行为，
      源码哈希本来就覆盖，编译器一行没改。CI 的 `guard-compiler-fingerprint` 用 base 的 stdlib 源码
      比两个编译器的输出 ⇒ 输出一致、自然过。
...
> ——那次「0 命中」是缓存骗的），看命中。
- [x] 4.1 全仓跑，收集所有新抛出的点
- [x] 4.2 逐个判读：是真 bug（该抛）还是需改写的合法写法
- [x] 4.3 结论写回 proposal

## 阶段 5
- [x] 5.1 `semantics.rs` 的 `bail!("InvalidCastException: …")` → 真异常（用户可读类型名）
- [x] 5.2 确认反射 / 泛型擦除路径也走到真异常
- [x] 5.3 JIT 侧：grep 是否另有 cast 快路；有则同步

## 阶段 6
- [x] 6.1 e2e 用例覆盖 spec 全部场景，**interp + jit 双模式**
- [~] 6.2 跨包接口不误抛的用例 → 随 follow-up（引用类型那支才需要）
- [x] 6.3 `xtask test all` + `cargo test --lib` + docs + examples
