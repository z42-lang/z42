...
实现期确认了比"性能退化"更硬的一条：**`pending` 在本变更之后永远归不了零。**
`register` 是**加载期急切**的（每个带 `$Cctor` 哨兵的类型入表即登记），而初始化是**惰性**的
——一个**从未被使用**的类型永远停在 `NotRun`，计数永远减不掉。本变更把带 cctor 的类型
从全仓 0 个变成上百个 ⇒ 门再也关不上。

**这不是门变慢，是这个门的设计前提被推翻了。** 全局计数只能表达"全程序有没有事要做"，
而本变更之后"有事要做"恒真；要表达的其实是"**这一个类型**有没有事要做"。

改为：**状态住在 `TypeDescCold` 上的一个 `Arc<AtomicU8>`**（7.3 已实施）。

```rust
// ensure_type_init 入口
if td.init_done() { return Ok(()); }   // 一次 relaxed load，无锁无查表
```

三条不变式（注释里逐条写死）：

1. **只是快路提示，不是真相来源** —— `CctorRegistry::map` 仍是唯一真相，只在它判定
   `Done` 之后才置位；快路只信 `true` 方向，`false` 一律走慢路复核。
2. **只单调 0→1，且只有成功才置位** —— 失败的类型必须每次回慢路才能命中 `claim` 的
   `Failed` 分支重新抛出（C# 语义：失败是终态、永不重试）。置位了就再也抛不出来。
3. **粒度是类型而非全程序** —— 正因如此不受"有类型从未被使用"拖累。

用 `Arc` 是因为 `TypeDescCold` 派生 `Clone`：克隆共享同一原子语义正确；即便某条路径
拿到独立副本（值 0），也只是多走一次慢路，不影响正确性。
...
| D7 | **编译器侧（7.1/7.2）与运行期侧（7.3/7.4）必须同一个 PR 落地，不得拆分** | 7.1 把 stdlib 里大量静态表（如 `TokenKind` 的 94 个 `static int`）从"加载期急切初始化"改成"首次使用才初始化"。运行期屏障在 7.3 改造完成前存在覆盖缺口，单独落 7.1 会让整条工具链处于"静态表可能未初始化"的状态 |

## 5.1 实施纪律（2026-09-22 实证，非理论）

- **必须在专属 worktree 里做**（[parallel-development §0 铁律](../../../agent/rules/parallel-development.md)）。
  本变更第一次实施时在主树 `z42-test` 上开分支，撞上另一会话正在同树进行的
  `use-free-function-overloads`，结果是一个与本变更**无关**的编译器崩溃
  （`ArraySet index: ... got Char('(')` @ `Lexer._sym`）被错误地归因到本变更上，
  排查了近一小时。**退回对照**（stash 后重建，崩溃照旧）才把归因纠正过来。
- **种子与源码的格式版本必须先对齐**（tasks 0.4.0）。否则 `build all` 的
  `seed cold-start` 会 stage 旧格式产物，被新 VM strict-pin 拒绝，而错误信息指向格式、
  不指向种子。
- **量测必须串行**。并发跑 bootstrap 时测得的编译器吞吐是 `3.681 s ± 1.237`，
  串行是 `1.906 s ± 0.044`——前者的噪声足以淹没本变更的任何真实影响。

## 5.2 落地顺序（受自举纪律约束）

| PR | 内容 | 前置 |
|---|---|---|
| **#1（本 PR）** | 7.1 编译器发射统一 + 7.3 运行期屏障三缺口 + 防回退门 | 无 |
| — | **发布一版含 7.1 的 nightly** | #1 合并 |
| **#2** | 7.4 删运行期 `__static_init__` 平行管道 | 种子产物中 `__static_init__` 计数为 0 |
| **#3（可选）** | 7.2 编译期屏障消除位 | 届时实测证明仍有收益 |

7.2 之所以降为可选：proposal 阶段的收益估算建立在「全局门 + 字符串切分 + 两次哈希查表」
之上，而 7.3 已把热路径压到**两次 relaxed load**，编译期再消除的边际收益大幅缩水；
而它要动 IR 指令编码 + 新 pass + 跨包保守规则，半径比 7.1/7.3 加起来还大。
**先量再做**，别按已经过期的前提立项。
