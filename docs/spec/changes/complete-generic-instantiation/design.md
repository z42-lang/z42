...
### D4：合成实例化产物的跨模块重复（**0.1 已实测定性——这条是 P1 的先决条件**）

原先只是「待验风险」，2026-09-23 查实后**升级为必须先解决的阻断**，设计随之修正。

**碰撞形态**（比原先设想的广得多）：不只是「两个依赖包都用了元组」——**「一个库内部用了
`(int,string)`，主程序也用了」就已经撞上**。两边各发一份 `Std.ValueTuple2<Int32,String>`
描述符 + 一份同名合成 ctor。P1 之后这是**极常见**形状，不是边角。

> 对 #774 不存在：本包实例化名带各自命名空间（`X.Pair<…>` vs `Y.Pair<…>`），天然不同。
> 是跨包特化把「名字属于生产方、描述符由每个消费方各发一份」这件事引进来的。

**实测三条路径**：

| 路径 | 行为 |
|---|---|
| `struct_alloc`（元组分配） | ❌ 不查歧义表，只 `try_lookup_type` 取布局 ⇒ 不受影响 |
| 类型描述符重复 | ⚠️ `registry.rs:83` warn + `note_ambiguous_type` + **first-wins 丢弃第二份** |
| **合成 ctor 等成员函数重复** | 🔴 `note_ambiguous_function` → `exec_call.rs:235-239` **调用即抛** |

编译期 E0601 **不会**误报：`PkgCheckFqn` 对实例化类型返回**定义**的 FQN（`Std.ValueTuple2`），
而那个名字只有 z42.core 声明；实例化名 `Std.ValueTuple2<Int32,String>` 从源码引用不到。
⇒ 问题全在运行期。

**修正（D4-fix）**：把「合成实例化产物」与「用户声明」在加载器里区分开。

合成产物是 `(定义, 类型实参)` 的**确定性函数**——两个消费方算出的描述符与成员体逐字节相同。
第二份不是歧义，是**同一件东西的第二次到达**。故在 `registry.rs` 的**函数循环与类型循环**里：

```
名字是实例化产物  ∧  与已在表里那份结构一致   ⇒ 静默跳过（不 warn、不记歧义）
名字是实例化产物  ∧  结构不一致               ⇒ 保持今天的歧义行为（不静默吞）
其余                                          ⇒ 保持今天行为，一行不改
```

- **判据「名字含 `<`」是可靠的**：用户源码写不出带尖括号的类型名——泛型定义按**裸名**发，
  arity mangle 走 `Name$N`，伪实例化（实参仍是型参）已被 `_isConcreteTypeArg` 排除。
- **结构断言**（size + 字段数 + 逐项偏移）是兜底：若两个消费方是对着**不同版本**的生产方
  编的，布局可能真不同。既有的包身份/版本门（`fix-package-identity-gate`）本该拦住这种组合，
  但断言让它即使漏过也**响而不是静默取第一份**。

⚠️ **这条改的是运行期加载器**，与「P1 运行期零改动」的原计划冲突 —— spec 的 Pipeline Steps
已同步更正。
