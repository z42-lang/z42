...
- [x] **S1-f** 跨 CU 的泛型体与泛型类型实例化（#774 既有缺陷）
...
**三条路（User 裁决走甲；实施时取了等价但小得多的做法，见下）**：
...
### 实施：没拆两阶段流水线，改为「只给声明了泛型的 CU 预算 model」

甲的正统做法是拆两阶段（先 Infer 全部 CU，再带所有 model 做 codegen），但 `BuildPackageCus`
那条路上**并行 / 诊断 / 缓存**缠在一起，改动面与风险都大。

实际取的做法等价而小得多：**只为「声明了泛型的那些 CU」预先算一遍 model**，连同 AST 一起
存进包级登记表（`GenericBodySrc`）。这些 CU 稍后编自己时会再 Infer 一次（多算一遍，不改变
任何结果），换来**流水线结构一行不动**。泛型声明文件在一个包里只是少数，代价有界。

发射侧新增 `IrGen.SpecBodyModel`：特化期间把「取绑定后的体」的 model 换成**声明所属 CU** 的，
其余一切不变。

✅ **A2（跨文件虚覆写）随之验成**：`Base` 在一个文件、`override` 在另一个文件，
`b.Bump<P2>(t)` 读出 **107**（S1-f 前是 `MissingSymbolException`，更早在 main 上是静默的 102）。
