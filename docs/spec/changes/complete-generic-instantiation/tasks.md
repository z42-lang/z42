...
- [x] **S1-d** **虚/override** 实例泛型方法（运行期「特化名不沿基类链继承」+ 本 CU 内覆写闭包）
...
🔴 **已知边界：跨编译单元的覆写看不见** —— `IrGen` 由 `CuCompile._compileCu` **按 CU** 创建，
拿不到别的文件里的 AST。那种接收者会命中第 1 条 ⇒ **大声报错，不会静默错**。
完整覆盖需要**包级特化通道**（跨 CU 收集工作项后统一发射），量级中等，记入下方遗留。

## 遗留

- [ ] 包级特化通道：跨 CU 的覆写特化（去掉 S1-d 的已知边界）
