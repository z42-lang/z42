...
- [ ] **Q1（主裁决）**：6 个根因与"开检查"同一个 PR，还是拆成前置 PR 阶梯？
      - **A（推荐）**：**拆两步**。PR-1 = R1（型参身份，占 84%，独立可测：跨包泛型调用今天就错）
        + R2/R7（赋值上下文已能复现的既存 bug）；PR-2 = 开检查 + R5/R6 + 负例门。
        理由：R1 半径最大且**与本检查无关也该修**（它今天已经在污染 JIT 类型信息）；
        先落 R1 能让 PR-2 的 diff 只剩「接线 + 门」，评审面小、rebase 成本低。
      - **B**：一个 PR 全做。原子、一次到位，但 diff 跨 8 文件 + 6 个根因，rebase 风险高
        （main 被并发会话高频推进）。
- [ ] **Q2**：R6（enum ↔ 整数）放行到什么程度——隐式双向、还是要求显式 cast（对标 C#）？
      这是**语言语义裁决**，不是实现细节。
- [ ] **Q3**：构造器实参同批接类型检查（design D4 已倾向"接"），确认？
