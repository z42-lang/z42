...
> 工作有两个主要来源（`Z42_GC_PHASES` 的 `card seed` / `minor roots` 两行可见）：卡表扫描是
> O(老年代)（`z42c.semantics` 最坏一次扫 **280 382** 个老条目），以及增量 major 的 grey 队列是
> minor 的根（`13_gc_large_heap --large` 上 **108 142** 个老条目，每次 minor 重新遍历一遍）。
