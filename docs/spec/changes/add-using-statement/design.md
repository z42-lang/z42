...
### ✅ 已实测（2026-09-26，本批开工第一件事）：今天**能**声明得出来

给 `z42.io/TextReader` 与 `z42.net/TcpClient` 临时加 `: IDisposable` ⇒ **`build stdlib` 25/25 全绿**。
⇒ 「当年放弃声明是撞了批 A–F（#778/#779/#784/#788/#789/#792）修掉的那批跨包接口缺陷」这个猜测坐实，
那些缺陷已修好，**跨包实现 `z42.core` 的接口现在成立**。

⭐ **并且校验真在跑**（反向探针，不是「声明了且编过」就算）：把 `TextReader.Dispose` 改名 ⇒
`E0412: TextReader implements IDisposable but does not define member Dispose`。
⇒ 名义声明是**被强制**的，不是装饰。这条是 D3-1 选「纯名义」的前提，已坐实。
...
## D3-1（**已裁决：甲 = 纯名义**，User 2026-09-26）：可 dispose 的判定

**裁决：纯名义 —— 只认 `: IDisposable`，对齐 C#。** 配套裁决：**库侧账并进本批一起做**
（给那 10 个资源类型补基表 + 删两句假注释）⇒ 「落地当天对 z42 自己的资源类型不可用」这个
唯一的反对理由被同批消掉。前提已实测坐实（见上：能声明得出来，且校验真在跑）。

⇒ 判据 = 「该类型的接口闭包里有 `Std.IDisposable` 吗」，**不是**「有 `Dispose()` 方法吗」。
⚠️ 仍必须问**闭包**而不是直接基表：`class A : B`、`B : IDisposable` 时 `A` 也可 dispose
（同 #827 的教训，只是这次问的是接口闭包而非成员表）。

### 原始三选（留档）：可 dispose 的判定用名义、形状，还是有序回落链？
...
## D3-2（**已裁决：四种全做**，User 2026-09-26）

含 `using T v = expr;`（显式类型的简化形态）⇒ **必须做前瞻消歧**：`using` 后是标识符时，
要区分 `using Stream s = …;`（语句）与 `using Foo.Bar;` / `using X = Y;`（import）。
落法 = 复用 `_isVarDeclStart` 那套「跳过类型 → 见标识符 → 见 `=`」的前瞻（`StmtParser:199-295`
已有 `_skipTypeOffset` 可用），**判据写在一处**、别造第二套。
⚠️ 这条把 D3-4 的消歧从「只看一个 token」升级成「有限前瞻」⇒ 门必须多一组：
`using X = Y;`（别名 import）与 `using T v = e;`（语句）在方法体里各自的归属。
...
## D3-6（**已裁决：`using_stmt`**，User 2026-09-26）

**`using_stmt`** —— 让 #783 删掉的那个**幻影名以真名回来**（那张表当年宣告它存在而实际没有，
