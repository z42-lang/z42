...
1. **P1 是否可以直接做**：终结器目前**零生产调用方**。侧表化保持对外 API 不变，
   但如果你打算近期给 z42 侧加 `IDisposable` / 终结器语义，侧表的每次查表成本
   会落在 sweep 上（sweep 已经是 O(存活对象)，多一次哈希查）。要不要现在就做？
2. **三项一起上还是拆成三个 PR**：我倾向**拆三个**——每项都有独立的改前/改后数字，
   且 P2 动 `ScriptObject` 布局风险最大，单独一个 PR 便于回滚。
