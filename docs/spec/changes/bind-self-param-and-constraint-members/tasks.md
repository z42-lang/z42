...
- ⚪ 2.2 `MemberResolver` 接口收者分支：签名形参位含 `Self` → E0454 + 提示改用 `where T : I`
- ⚪ 2.3 改写 `src/tests/cross-zpkg/self_type_cross_pkg/ext/src/Ext.z42` 的 `eqVia`
  → `bool eqVia<T>(T a, T b) where T : IEq`（顺带首次覆盖 Part A 的**跨包**约束查找）
- ⚪ 2.4 单测 + 退回对照（真门 + 无误报守卫：`getVia(IBox<int>)` 这类无 `Self` 形参的不得被误报）
- ⚪ 2.5 完整 GREEN

## 阶段 3 —— 文档与归档（⚪）

- ⚪ 3.1 `docs/book/src/language/generic-constraints.md`：`Self` 实现模型节补形参位规则；
  新增「型参收者的约束成员绑定」小节
- ⚪ 3.2 `docs/roadmap.md`：关掉本条 Deferred；**新登记** `tighten-bare-type-param-target-erasure`
