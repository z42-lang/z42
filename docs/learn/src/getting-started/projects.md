...
> `z42 new` 还能建库工程等其它形态，全部选项见参考手册的
> [`z42` 命令参考](https://z42-lang.github.io/z42/reference/toolchain/cli-z42.html)。
...
清单能写的远不止这两段——依赖、构建选项、测试目标、各平台配置都在里面。本章只用到最少的几个字段，
**完整字段表**见参考手册的 [工程清单 z42.toml](https://z42-lang.github.io/z42/reference/toolchain/z42-toml.html)。
...
**缓存**按配置分开（`artifacts/debug/` 与 `artifacts/release/`），所以在两种配置之间来回切换
不会互相作废对方的缓存；但**产物共用 `dist/`**——后构建的那次会覆盖前一次。

## 运行构建产物

`dist/` 下的 `.zpkg` 就是构建结果，可以直接交给 `z42 run`：

```console
{{#include ../../../../examples/getting-started/projects/greeter/run.console:run-artifact}}
```

和在工程目录里跑 `z42 run` 的效果一样——区别只是**不需要源代码和清单**，把这个 `.zpkg`
拷到别的机器上（装了 z42 的），照样能跑。

debug 构建还会在 `dist/src/` 下留一份每个源文件单独编译出的 `.zbc`：

```
dist/
├── greeter.zpkg        ← 包：可直接运行
└── src/
    ├── Main.zbc        ← 单个源文件的字节码
    └── Greeting.zbc
```

**`.zbc` 不能单独运行**——程序入口（从哪个 `Main()` 开始）记录在 `.zpkg` 里，
裸 `.zbc` 没有这个信息，直接跑会报 `no entry point`。日常只需要认准 `.zpkg`。

> 想把程序交给**没装 z42** 的人，需要的是**原生可执行文件**，那要用 `z42 publish`，
> 本手册后面的「发布应用」一章再讲。
...
| 目录 | 装什么 | 要提交到版本库吗 |
|------|--------|-----------------|
| `dist/` | **交付产物**——拿去运行或分发的东西 | 不要 |
| `artifacts/` | **增量编译缓存**，按配置分子目录（`debug/`、`release/`） | 不要 |

两个都是**可重生的**：删掉之后重新构建一遍就回来了，区别只是删了 `artifacts/` 下次要全量重编。
`z42 new` 生成的工程自带 `.gitignore`，已经把它们排除在外。
...
|---|---|---|
| `<工程名>.zpkg` | ✅ | ✅ |
| `dist/src/*.zbc` | ✅ 每个源文件一份 | — |
| `<工程名>.zsym` | — | ✅ **调试符号**：程序崩溃时用它把地址还原成文件名与行号 |

> `.zsym` 要和产物一起保留（但**不必**和程序一起分发）——出问题时没有它就只能看到裸地址。
