...
C# 里 `$"{x:X2}"` 能把数字按十六进制两位输出。**z42 不支持这个写法，会直接报错**：
...
需要定宽或换进制时，自己拼接或调用相应的转换方法。

> 📜 **历史**：2026-09 之前这里是**静默的**——冒号后面的部分被直接丢掉，`{r:X2}` 原样打出
> `255`、`{3.14159:F2}` 打出 `3.14159`，一条诊断都没有。「不支持」必须说出来：静默失效比
> 不支持更糟，因为你会以为它生效了。
