...
2. **运行到没有匹配臂的情形时，`switch` 表达式抛 `SwitchExpressionException`**。
   消息里带着**落空的那个值**（上面是 `West`），抛出位置在栈回溯里。

表达式必须产出一个值，没有臂可采纳时就没有值可产——所以这里只能抛。**穷尽性仍然要靠你自己
保证**：要么补齐所有情形，要么加一条 `_ =>` 兜底臂。区别是漏掉的话会当场炸，而不是把一个
坏值交给你。

> ⚠️ **警告只覆盖封闭域**（`bool` / `enum` / 封闭类层次）。subject 是 `int` / `string` 这类
> **开放域**时编译器不可能穷尽，于是一条警告都没有：
>
> ```z42
> int a = n switch { 1 => 10, 2 => 20 };   // 编译期静默；n 是别的值时运行期抛
> ```
>
> 这也是为什么必须在运行期抛——光靠警告盖不住这一半。

> 📜 **历史**：2026-09 之前这里是**静默产出 `null` 或垃圾值**（`int` 变量能打印成 `null`，
> `a + 1` 打出 17179869186 这种数）。错值会一路流到很远的地方才以别的形态崩，
> 是最难查的一类 bug。

`switch` **语句**不受这条约束——语句不产值，没有匹配的 `case` 就什么都不做，不抛。
...
- 🔴 **穷尽性在编译期只是警告**（而且只覆盖封闭域），漏掉的情形在**运行期抛
  `SwitchExpressionException`**——自己补齐，或者加 `_ =>` 兜底。`switch` 语句不抛。
