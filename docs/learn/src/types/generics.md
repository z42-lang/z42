...
### 静态方法里 `typeof(T)` 拿不到类级型参

类级型参的实参有两个来源：**实例自己带着**（每个 `Box<int>` 对象记着 `T = int`），
或者**写在类型名里**（`Derived : Box<int>` 的基名就是 `Box<int>`）。
所以实例方法里 `typeof(T)` 是对的，继承来的型参也取得到 —— 只有**静态方法**两个来源都没有
（没有实例，调用点也不把类级实参传进去），只能退回占位名 `"T"`：
...
- **静态方法**里没有实例 —— 问不到，得 `"T"`。同一语境下 `default(T)` 得 `null`
  （它**宁可给不出值，也不会去猜**：静态方法的第一个实参恰好是个泛型对象时，
  曾经会静默读到那个实参的实参表、给出一个看着对的错值）。

> 📜 这一节原本还列着两条，都**已经修了**：
> - 「类级 `typeof(T)` 一律给不出真类型」——实例语境下现在就是 `Std.Int32`，
>   `typeof(T) == typeof(int)` 为 `true`。
> - 「继承来的型参取不到」——`Derived : Box<int>` 的 `typeof(T)` 现在是 `Std.Int32`，
>   同一实例上 `default(T)` 也是 `0` 而不是 `null`。实参一直都在**基的名字里**，
>   只是此前被擦掉了。
>
> 剩下的只有静态方法这一种「两个来源都没有」的场景。
