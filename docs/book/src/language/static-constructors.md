...
> ⚠️ **已知缺陷（2026-09-14 发现，未修）**：**跨包**时，若对依赖包里某类型的**第一次**使用是「调用它的静态方法」
> （含静态属性访问器），它的静态构造器不会执行——依赖包惰性加载时 cctor 只在 `try_lookup_type` 登记，函数查找
> 不走那里，屏障的 `any_cctor_pending()` 门因此恒假。同包、以及先读写静态字段 / `new` 的跨包用法不受影响。

同一个类可以同时声明静态构造器与无参实例构造器；静态构造器**不参与**实例构造器的选择、E0426 实参个数校验
与 `where T : new()` 判定，见[实例构造器与初始化子句](constructors.md)。
