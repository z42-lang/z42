...
| `E06xx` | 包 / 导入解析 | `E0601` 跨包同全限定名（见下）；`W0603` / `W0604` / `W0606` 导入相关警告 |
| `E09xx` | Native / 测试 | `[Native]` FFI 约束；`[Test]` 家族的位置 + 签名强制（`E0911`/`E0912`/`E0915`，见下） |
| `E10xx` | 调用实参绑定 | `E1001` / `E1002` 参数绑定错误 |

> **`E0601` / `W0606`（跨包同全限定名）**：两个**不同的包**声明了同一个 FQN 时，写全限定名
> 也分不开它们（两者逐字相同），输的那一份在 z42 里**没有任何写法能指到**——故 `E0601` 在
> **使用位**报错（对标 C# CS0433；未被引用则不报，避免下游被两个第三方依赖的冲突挡死）。
> 本包声明的类型遮蔽了某个导入包的同 FQN 类型 → `W0606` **warning**（本地恒赢是既定规则，
> 对标 C# CS0436），修法是改名而非加限定。与 `E0456`（同短名跨 ns，限定写法即可消歧）、
> `E0458`（同包内重复声明）三者的分工见
