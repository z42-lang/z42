...
| `E06xx` | 包 / 导入解析 | `E0601` 跨包同全限定名、`E0606` 本包遮蔽导入（见下）；`W0603` / `W0604` 导入相关警告 |
| `E09xx` | Native / 测试 | `[Native]` FFI 约束；`[Test]` 家族的位置 + 签名强制（`E0911`/`E0912`/`E0915`，见下） |
| `E10xx` | 调用实参绑定 | `E1001` / `E1002` 参数绑定错误 |

> **`E0601` / `E0606`（跨包同全限定名）**：两个**不同的包**声明了同一个 FQN 时，写全限定名
> 也分不开它们（两者逐字相同），输的那一份在 z42 里**没有任何写法能指到**（C# 有 `extern alias`
> 作逃生口，z42 没有）——故两者都**报错**，修法只能是改名 / 换命名空间。
>
> 二者的差别只在**谁能修**：`E0601`（两个**依赖包**互撞）在**使用位**报（对标 C# CS0433），
> 未被引用则不报——那是两个第三方依赖打架，下游既没用到也无权修，不该挡死他；
> `E0606`（**本包**声明的类型遮蔽了导入包的同 FQN 类型）冲突的两份里有一份是本包自己写的，
> 改名随时可以 ⇒ 直接报错可行动。**注意 `E0606` 与 C# CS0436 的取舍不同**：C# 给 warning，
> z42 给 error，因为 z42 没有 `extern alias` 这类逃生口，"本地恒赢"下被遮蔽那份**永远调不到**，
> 失败形态退化成「no method X on Y」这种答非所问的诊断。
>
> 与 `E0456`（同短名跨 ns，限定写法即可消歧）、`E0458`（同包内重复声明）三者的分工见
> [source-compile.md](source-compile.md)。
>
> **同一组码也覆盖自由函数**（report-crosspkg-duplicate-function）：自由函数的符号表键是**裸名**、
> 连 FQN 视图都没有，且只有裸名一种调用形态 ⇒ 输的那份**永远指不到**，运行期还常常连 warn
> 都没有（输家因惰性加载不被载入）。同 ns 跨包 → `E0601`；同 ns 本包遮蔽导入 → `E0606`；
> 不同可见 ns → `E0456`。
>
