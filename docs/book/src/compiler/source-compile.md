...
**E0601 的触发时机是使用位**（对标 C# CS0433）：两个依赖包撞了、而本包一行都没引用它 → 不报。
...
>
> 与 E0601「未引用则不报」的分工也不同：那条是**两个第三方依赖打架**，下游既没用到也无权修；
> 本条冲突的两份里**有一份是本包自己写的**，改名随时可以 ⇒ 报错是可行动的。唯一代价是
> 「故意声明同 FQN 以覆盖库类型」这条野路子，而它本就无法控制、无法回退（指不到被覆盖那份）。
> 实测全仓 0 处。
...
所以自由函数这侧**编译期是唯一防线**。三种形状各归其码（不新开码，语义正好对上）：
...
判据全在 `SymbolTable`（`DuplicateFuncPkgMsg` / `ShadowedImportFuncMsg` / `AmbiguousFuncNameMsg`），
数据来自 `FuncOriginAll`：**函数名 → 全部来源 `ns#pkg`**（`pkg` 为空串 = 本包）。为什么必须把
ns 与 pkg 一起存：三种形状恰好由「ns 同不同」×「pkg 同不同」区分，只存其一都判不出来。
