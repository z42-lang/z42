...
2. **优化本身的时间/内存开销要有度**:
   ...
   - 运行时分析(加载期/JIT 期)不能拖垮加载与执行;能编译期算好的静态分析就别留到运行时反复算(见 [self-hosting](../../../internals/src/compiler/self-hosting.md) 的 REGT 先例:reg_types 编译期算好,运行时只消费)。
...
## 关联文档
- 自举与 REGT 先例(编译期算好、运行时消费的模式):[self-hosting](../../../internals/src/compiler/self-hosting.md)
