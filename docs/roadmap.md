...
| 0.3.12 | **反射完整化**：~~Method.Invoke（非泛型）~~ ✅ + ~~Type.GetType(fqn)~~ ✅（add-method-invoke-non-generic；builtin 复用 exec_function，异常原类型传播 interp+jit；Activator 无参延后）‖ ~~IsEnum~~ ✅（2026-07-09）‖ 嵌套泛型 GetGenericArguments ‖ ~~接口成员枚举~~ ✅（2026-07-20 add-interface-member-reflection，纯 runtime surface zbc 1.28 接口方法块）| 🟡 Invoke/GetType/IsEnum/接口成员枚举 完成；嵌套泛型 args 余 | |
