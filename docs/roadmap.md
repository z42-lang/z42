...
| `repl-indent-future-grid-snap-backspace` | REPL 退格 floor 到前制表位（错位一键归正）——需 `Replace(WholeLine)`（唯一 redo-免疫的变量宽度删+插），但 rustyline `edit_insert_text` 插入后不推进光标 → 光标归位行首，破坏后续输入。需 patch `edit_insert_text`。Tab 方向已由 `add-repl-tab-grid-snap` 落地（`Insert`，光标正确）；退格仍走定量一级 `Dedent` | [archive/2026-08-22-add-repl-tab-grid-snap/design.md](spec/archive/2026-08-22-add-repl-tab-grid-snap/design.md) Deferred 段 |
| `repl-indent-future-rbrace-dedent` | REPL `}` 自动回退一级——可用 redo-免疫的 `Replace(WholeLine, "<dedent>}")` 实现（**无需 fork**，修正 #253 判据），但 rustyline `edit_insert_text` 不推进光标 → 光标归位行首，破坏 `}` 后继续输入（`} else {`）。需 patch `edit_insert_text` 或多行编辑重构后重估。workaround：手动退格一次（`Dedent` 一次到位、光标正确）| [archive/2026-08-22-add-repl-tab-grid-snap/design.md](spec/archive/2026-08-22-add-repl-tab-grid-snap/design.md) Deferred 段 |
| `repl-indent-future-paste-reflow` | REPL 多行粘贴回改任意行——属逐行 readline 架构，依赖真多行编辑（rustyline Validator）重构；spike 已确认粘贴本身无双重缩进/损坏 | [archive/2026-08-22-add-repl-indent-editing/design.md](spec/archive/2026-08-22-add-repl-indent-editing/design.md) Deferred 段 |
