...
| struct 堆内联 P3b follow-up（add-struct-heap-inline）| **已落地**：struct 真内联进**堆对象字段**（`class C{ Point pt; }`；D1-a 基元字节内联 `struct_bytes` + 引用叶子 `struct_refs` 侧表 + 复用 `StructFieldGetPrim/SetPrim` 对象 base 路线 α + GC scan/`write_barrier_field` 复用侧表 + 格式 wire 内联字段表 zbc1.32/zpkg0.37）。**剩余**：① **`struct[]` 字节 backing 元素 codegen**（runtime `ArrayBacking::StructBytes`+`Value::StructRefHeap` 句柄+exec+GC **已实现并单测**；缺 `arr[i]` 产句柄的 codegen——`ArrayGet` 对 struct[] 出 `StructRefHeap` + 元素 leaf/copy 访问）② **class 方法返回 struct**（`Point GetPt(){return pt;}`——`sret × VCall` 接线，当前 sret 只覆盖 struct-receiver 方法 + 静态/自由调用）③ **JIT 值路径**（P5，现全 bail→interp）④ **跨包内联布局 + 反射**（P4）| [book struct-value-semantics](book/src/runtime/struct-value-semantics.md) 「堆内联」段 + [archive/…-add-struct-heap-inline](spec/archive/) |
