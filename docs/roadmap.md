...
| `Self` 返回位不做替换（self-return-type-substitution）| add-associated-types PR-2 的 `Self` 是「隐式型参」模型：经**接口静态类型**调用返回 `Self` 的方法，结果类型就是型参 `Self` 本身（`IClone c; var x = c.Copy();` → `x : Self`），不替换成接收者静态类型、也不随 `where T : IClone` 的实参代入。具体类上调用不受影响（走实现方签名）。做替换属表达力下一档 | [book: generic-constraints.md](book/src/language/generic-constraints.md) `Self` 类型 §实现模型 |
