...
| `repl-indent-future-grid-snap` | REPL 缩进网格吸附（错位缩进一键归正）——rustyline redo 覆盖 movement 计数，变量计数删除做不到 redo-免疫；需 patch rustyline。当前为定量一级 Indent/Dedent | [archive/2026-08-22-add-repl-indent-editing/design.md](spec/archive/2026-08-22-add-repl-indent-editing/design.md) Deferred 段 |
| `repl-indent-future-rbrace-dedent` | REPL `}` 自动回退一级——需「删一级+插入」单命令，rustyline `Replace` 计数被 redo 覆盖；随多行编辑重构一并做。workaround：手动退格一次（已成级 dedent）| [archive/2026-08-22-add-repl-indent-editing/design.md](spec/archive/2026-08-22-add-repl-indent-editing/design.md) Deferred 段 |
