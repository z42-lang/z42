...
| 裸型参作转换目标时的擦除（tighten-bare-type-param-target-erasure）| `Conversion._classifyBuiltin` 分支 B「恰一侧含泛型形参 → 擦除放行」对「**目标是裸型参、来源是具体类型**」也放行 ⇒ `bool bad<T>(T a) where T : IEq { return a.Same("nope"); }` 今天仍无诊断（C# 对应诊断是 CS1503）。⚠️ 这使上一行 Part A 的实参检查**只覆盖形参类型是具体类型的成员**；收紧它是对**通用**擦除规则动刀，爆炸半径未量 | [book: generic-constraints.md](book/src/language/generic-constraints.md) §已知限制：形参本身是型参时仍不检查 |
| 跨包泛型自由函数的型参保真度（imported-generic-func-type-param-fidelity）| **跨包**调用泛型自由函数今天编不过：`int idOf<T>(T a)` 在包 A、包 B 调 `idOf(p)` 报 `E0402: cannot assign Point to T (argument)`，**显式类型实参也不救**。导入侧型参退化成普通类（`_hasGenericParam` 判 false ⇒ 落结构比对）。与 `Self` / 约束无关（无约束的 `idOf<T>` 形态最简且同样炸）。同族于 #523 修的 `ImportedSymbolLoader` 四条类型保真度，但那批修的是**方法**、自由函数漏网。发现于 `bind-self-param-and-constraint-members` 改写 cross-zpkg fixture 时 | `ImportedSymbolLoader`；复现记在 `src/tests/cross-zpkg/self_type_cross_pkg/main/src/Main.z42` 注释 |
| where 约束：推断调用与顶层函数（where-constraint-future-inferred-method-args / -toplevel-func）| 方法级约束只在**显式**写类型实参时校验（`Max<int>(a,b)` 校验、`Max(a,b)` 不校验）；顶层 `FuncDecl` 的 `where` 完全不校验 | [book: generic-constraints.md](book/src/language/generic-constraints.md) 已知限制 §2/§3 |
...
| 驱动隐藏 warning（driver-hides-warnings）| `z42c.driver` `Main.z42` 的 `if (art.ErrorCount > 0)` 门控**全部**诊断呈现 ⇒ 无 error 时 warning 一条不打印（analyzer 警告同病）。「warning 探针先行」这类落地策略在本项目整体失效 | `src/compiler/z42c.driver/src/Main.z42` |
| **单文件编译无错误门**（emit-zbc-no-error-gate）| `IrDump.ZbcBytesD` 丢弃 `cm.ErrorCount`/`DiagMsgs`，`--emit-zbc` 对**任何**语义错误都退 0 并照常产出 .zbc（实测：未定义符号亦然）⇒ **golden 语料（`src/tests/**`）一直在零编译错误检查下编译**，「GREEN 全绿」不构成「无编译错误」的证据。影响面覆盖所有诊断，不止 where 约束。修它要先量爆炸半径（会点亮多少潜伏诊断） | `src/compiler/z42c.semantics/src/IrDump.z42` `ZbcBytesD` / `z42c.driver` `Main.z42` `--emit-zbc` 分支 |
