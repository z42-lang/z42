...
| where 约束：接口类型实参匹配（where-constraint-future-type-arg-matching）| 接口约束 v1 **只比裸名**——`IEquatable<string>` 也满足 `where T : IEquatable<T>`。与运行期同口径故无分歧；裸名匹配还顺带消掉 F-bounded 自引用（`INumber<T> where T : INumber<T>`）的无限递归 | [book: generic-constraints.md](book/src/language/generic-constraints.md) 已知限制 §1 |
