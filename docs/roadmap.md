...
| 跨包重名的自由函数 / 静态调用键（crosspkg-duplicate-free-function）| E0601 只覆盖**类型引用**这条链。自由函数与静态调用走 `DependencyIndex` 自己的活跃-ns 解析，同名跨包仍是静默 first-wins（运行期 `duplicate function ... keeping first-loaded` 会 warn，编译期不会） | [archive/2026-09-12-report-crosspkg-duplicate-type/tasks.md](spec/archive/2026-09-12-report-crosspkg-duplicate-type/tasks.md) Deferred 段 |
| 运行期 duplicate 是否升成 error（runtime-duplicate-symbol-severity）| `lazy_loader/registry.rs` 的 `duplicate type/function … keeping first-loaded` 今天是 `tracing::warn!`（默认打 stderr，**不哑**）。是否该改成致命错，取决于先厘清合法重复场景（静态链接 / 同包两副本 / merged main + lazy 同包）| 同上 |
