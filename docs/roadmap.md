...
| `ab-bench-micro`（Stage 2）| 把同-runner A/B 扩到 micro/stdlib tier（ns 级）；前置 = Bencher 加 mean/stddev（Welford）+ 自适应采样（confound 消除后 within-run SEM 才有意义）。Stage 1 只做 e2e | [changes/add-same-runner-ab-bench-gate/design.md](spec/changes/add-same-runner-ab-bench-gate/design.md) Deferred 段 |
| `ab-bench-criterion`（Stage 3）| criterion（Rust）tier 接入 xtask A/B 或砍占位（smoke_bench 空壳、gc_cycle_bench 只 `cargo bench` 直跑、无门禁）| [changes/add-same-runner-ab-bench-gate/design.md](spec/changes/add-same-runner-ab-bench-gate/design.md) Deferred 段 |
...
| `repl-multiline-future-rbrace-floor` | REPL `}` 自动回退一级 + 退格 floor 到前制表位（错位一键归正）——需 `Replace(WholeLine)`（唯一 redo-免疫的变量宽度删+插），但 rustyline `edit_insert_text` 插入后不推进光标 → 光标归位行首，破坏后续输入。整块多行模型（`add-repl-multiline-editing`）已落地，为其提供更干净地基（成为**当前物理行内**编辑），但仍需 patch `edit_insert_text` 光标推进。workaround：手动退格一次（`Dedent` 一次到位、光标正确）| [archive/2026-08-23-add-repl-multiline-editing/design.md](spec/archive/2026-08-23-add-repl-multiline-editing/design.md) Deferred 段 |
