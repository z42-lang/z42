...
| `SemanticDump` 丢弃 collector 相位诊断（semanticdump-skips-collector-diags）| `SemanticDump.FirstErrorCode`/`ErrorCount` 给 TypeChecker **另建** DiagnosticBag、**从不合并** `SymbolCollector.Diags`（SemanticDump.z42:154-157）⇒ collector 的 `_chkTypeRef` 发出的诊断（方法返回类型 / 形参类型 / 字段类型位置的未定义类型 E0443）在该 harness 里**恒不可见**。实测：`class C { Undef f(){…} }` / `void f(Undef x)` / `public Undef x;` 三者 FirstErrorCode 全为 ""，只有局部变量声明与 `new` 才报。后果：**任何以「签名位置写坏类型应报错」为断言的单测都是空测试**——add-associated-types PR-2 的 `Self` 用例首轮即撞上（改用 `DumpBody` 的解析结果作断言才成真门）。与 `emit-zbc-no-error-gate` 同族（诊断被丢在半路），修前须先量爆炸半径 | `src/compiler/z42c.semantics/src/SemanticDump.z42` |
