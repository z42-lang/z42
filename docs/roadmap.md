...
**模块整合新增项（2026-07-15，源 todo-list 第 11 行；four-streams 表未含，作 R/X/M 列补入）**：
...
| 能力 | 所属 minor | 当前状态 |
|------|:------:|:----:|
| 类型系统 / 内存模型 / 异常 / 类型定义 / 函数 / 控制流 / 字符串 / 集合 / 导入 / 数值别名 | 0.1.x | ✅ L1 |
| 可空性 —— 值类型永不可空（`int x = null` / `intExpr == null` / `int?` 均判错） | 0.6.x | ✅ enforce-value-type-non-null |
| 可空性 —— 引用类型 `?` 标记 + 流分析（形参 / 返回值 / 字段 / override·接口一致性 / `Expect` 逃生口；`?.` 与 `??` 已移除） | 0.6.x | ✅ define-null-check-marks（跨包携带见该 change §3.3 的三类已知缺口：接口成员 / 字段 / extern 桩） |
...
| 跨平台 SDK package 分发 | 0.2.6 | 13 个 per-arch SDK 包（desktop × 5 / iOS × 3 / Android × 4 / wasm × 1）；统一 `bin/libs/native/manifest.toml` 形态（examples 已于 2026-06-20 移出发行包）；详见 [embedding.md §11.9](internals/src/runtime/embedding.md#119-分发-package-形态per-arch-flat2026-05-13-define-package-layout) |
...
| 特性 | 描述 | 在哪里 |
|------|------|------|
| 关联类型跨包（assoc-type-crosspkg）| 同包已实现（add-associated-types PR-3）。跨包**不校验**：类给出的绑定与接口的关联类型名单都无 wire 表示，需要约束 bundle 的 **bit7**（实测 bit7 **未被三方 reader 预留**，`type_reader.rs:296-335` 只解析 bit0–6 ⇒ 与 PR-1 不同，这次是真 wire 变更）+ **zbc/zpkg 双 minor bump** + `ExportedClassZ`/`ExportedInterfaceZ`/`TsigReconcile` 三段搬运。⚠️ 落地前编译器在三个点**主动跳过**导入类型（约束声明处 / 使用点 / 补齐强制）——不跳则合法跨包代码被判「未绑定」**假红** | `ConstraintChecker._checkAssocBinding` / `_fillBundle` / `InheritanceResolver._checkOneIfaceAssoc` 的 `IsImported` 守卫 |
| 关联类型：嵌套约束（assoc-type-nested-constraint）| `where T : IIterator<Item = U>, U : IDisplay` —— 绑定引出的类型再被约束。同包关联类型已落，此项未做 | [book: generic-constraints.md](reference/src/language/generic-constraints.md) 已知限制 §5 |
| where 约束：接口类型实参匹配（where-constraint-future-type-arg-matching）| 接口约束 v1 **只比裸名**——`IEquatable<string>` 也满足 `where T : IEquatable<T>`。与运行期同口径故无分歧；裸名匹配还顺带消掉 F-bounded 自引用（`INumber<T> where T : INumber<T>`）的无限递归。⚠️ add-associated-types PR-2 的 `Self` **只是绕开**（不写类型实参就没有实参可写错），**并未消除**：带实参的接口仍按裸名匹配，且 stdlib 现有声明尚未改写成 `Self` 写法（use 改写受 bootstrap-seed 轴① 约束，须等下一 nightly）⇒ 本条**继续开着** | [book: generic-constraints.md](reference/src/language/generic-constraints.md) 已知限制 §1 |
| ~~`Self` 返回位不做替换（self-return-type-substitution）~~ ✅ **已解决**（2026-09-07，change `self-return-type-substitution`）| 经**接口静态类型**调用返回 `Self` 的方法，结果类型现在是**该接口本身**（`IClone c; var x = c.Copy();` → `x : IClone`）——可靠上界，实现方必然实现该接口。具体类上调用不受影响（走实现方签名 → 具体类型）。**形参位**已于 2026-09-07 由 `bind-self-param-and-constraint-members` 收口（见下一行） | [book: generic-constraints.md](reference/src/language/generic-constraints.md) `Self` 类型 §实现模型 |
| ~~`Self` 形参位不做替换~~ ✅ **已解决**（2026-09-07，change `bind-self-param-and-constraint-members` Part B）| 结论是**禁止而非替换**：形参位 `Self` 是**逆变**方向，没有可用上界——把 `Self` 换成接口本身一个字也拦不住 `IEq a = new P(); IEq b = new Q(); a.Same(b)`（P 和 Q 都是 IEq），而实测放行的后果是派发到 `P.Same(P)`、`other.V` 从一个 `Q` 上读出 **null** 且静默返回 false。故经**接口静态类型**调用形参位含 `Self` 的方法一律报 **E0454**（对标 Rust object-safety），诊断消息直接给出替代写法 `f<T>(T a, T b) where T : IEq`——那条路 `Self ≡ T` 精确、且实参真校验 | [book: generic-constraints.md](reference/src/language/generic-constraints.md) `Self` 类型 §实现模型 |
| ~~型参收者不按 where 约束解析成员~~ ✅ **已解决**（2026-09-07，change `bind-self-param-and-constraint-members` Part A）| 此前 `MemberResolver` 的型参收者分支只查 `Object` 类，查不到就松绑 `sig = null`，而实参检查第一行就是 `if (sig == null) return;` ⇒ 泛型代码里对约束接口方法的调用**实参一律不检查、返回类型一律 `<unknown>`**（`PriorityQueue`/`SortedSet`/`Dictionary` 走的正是这条路）。现按型参的 where 约束接口（方法级 + 类级两来源、父接口闭包）解析出真签名；`Self` 在这条路上**精确**替换为该型参。实测欠债 = 0 | [book: generic-constraints.md](reference/src/language/generic-constraints.md) §型参收者上的约束成员绑定 |
...
| AST walker 完备性门（ast-walker-completeness-gate）| 🔴 `MethodTypeParamUse` 的遍历完备性靠 2026-09-08 从源码机械枚举建立（Expr 36 / Stmt 16 / Pattern 10 / TypeExpr 6），**AST 加新节点类不会让任何测试变红**，漏了就是静默洞。需要「节点类全集 vs walker 已处理集」的对账门 | `src/compiler/z42c.semantics/src/MethodTypeParamUse.z42` |
| where 约束：func 类型约束（where-constraint-future-func-constraint）| `where T : Func<int,R>` —— E0422/E0423 已定义但从未发出。⚠️ CallEmitter 靠该约束把参数当 func 值走 CallIndirect，改动需谨慎 | [book: generic-constraints.md](reference/src/language/generic-constraints.md) 已知限制 §4 |
...
| 加载上下文（ALC 式） | zpkg 重载/卸载/回收（含内部 metadata/缓存池）；惰性 GC 卸载 + 保留根诊断 + 缓存不自钉铁律 | [runtime/load-context.md](internals/src/runtime/load-context-design.md) |
| 诊断与跟踪 | 事件（编译/类型/GC/deopt/context）+ 计数（counter/gauge/histogram）+ 时间（per-函数编译耗时）；fire() 近零成本门控 + perfetto 输出 | [runtime/diagnostics.md](internals/src/runtime/diagnostics-design.md) |
| 统一 safepoint/STW + 精确 GC 契约 | GC safepoint 泛化为 OSR/卸载/hot-reload 共用；线程状态（InNative=安全）；精确 GC = GC map@安全点 + 派生指针受控（ALC 卸载前提） | [runtime/safepoint.md](internals/src/runtime/safepoint-design.md) |
| 对象与值表示 ABI | 隐式 Value/对象 ABI 固化（repr(C)+tag 表）；统一对象头去 native；字符串改 GC；移动/分代预留（gc_word/forwarding/card table/pin 区）；TypeDesc 留 context-arena | [runtime/object-abi.md](internals/src/runtime/object-abi.md) |
| ✅ 引用压 8B / `Value` 24→16B（B-radical 子目标，2026-08-11 提，**2026-08-15 落地 `unify-object-byte-layout` PR-3~5**）| 引用改平台指针大小（GcRef 16B→8B：**采路 A** 标记指针塞窄 generation 保非移动 GC；String `Arc<str>` 胖指针 16B→细指针 8B 长度进头；**FuncRef `Box<str>`→`Str` 8B**）→ `Value` 最大 payload 8B → enum 24B→16B（全 VM 密度/cache 33%）。**收益在密度非 native 交互**。全 VM 横切（JIT stride 24→`size_of::<Value>()`+`size_of::<Value>()==16` 编译期 pin）；不在 struct P3b。~~残留 follow-up：string 全 GC 化~~（✅ 已由 `unify-gc-heap` 完成，见下行） | [runtime/object-abi.md §2.1](internals/src/runtime/object-abi.md) |
| ✅ 统一 GC 堆（B-radical，**2026-08 落地 `unify-gc-heap` PR-1~5**）| string / closure / array backing 三类变长 payload 全收进单一 GC 变长块 region（`gc/var_region.rs`，A' 分配器）：PR-1 分配器原语 → PR-2 closure → PR-3 array backing → PR-4 string（ambient 堆 + lazy per-ctx interning + `Value::Str(VarGcRef)`）→ PR-5 收敛（fn_name 迁 GC Str 删 closure drop-glue + `visit_gc_children` 单一访问器 + 删 interned_strings 死代码）。消除 Arc/Box/外部 Vec 与 GC 的双重管理，为移动/压缩/去重 GC 铺路。前置 = 引用压 8B（上行） | [runtime/gc.md「变长块堆」](internals/src/runtime/gc.md)、[object-abi.md §5](internals/src/runtime/object-abi.md) |
| AOT 后端 | cranelift-AOT（复用 JIT 翻译 + cranelift-object，非 LLVM）；AOT+JIT+interp 混合（.NET R2R/ART 模型）；host 交叉编译；精确 GC stack map | [runtime/aot.md](internals/src/runtime/aot.md) |
...
| sidecar 跨目录搜索 | debuginfod 风格 + 环境变量配置 | [language/exceptions.md](reference/src/language/exceptions.md#deferred--future-work) |
| `Std.Reflection.Symbolicate` 公开 API | 程序内触发符号化 | [language/exceptions.md](reference/src/language/exceptions.md#deferred--future-work) |
| Facade threading 测试（R8）| 等 runtime threading 模型落地后回到 platform-test-contract 补"后台 invoke + 主线程 sink"scenario | [runtime/embedding.md §12](internals/src/runtime/embedding.md#§12-deferred明确不做的) |
...
| `slim-instruction-stringid` (E2.P3) | `String → StringId` intern 收敛，进一步缩小 payload struct（正交 slim-instruction-enum 之后）| [runtime/ir.md](internals/src/formats/ir.md#deferred--future-work) |
| ~~`self-hosting-future-indexed-zpkg`~~ ✅ | 已解决 (2026-07-08) — add-indexed-zpkg-min-patch：indexed 重定义实装（zpkg 0.24，主文件 FILE 目录 + 自包含散装 zbc + hash 校验 + VM 装载）| [compiler/self-hosting.md](internals/src/compiler/self-hosting.md#self-hosting-future-indexed-zpkg已解决-2026-07-08add-indexed-zpkg-min-patch) |
| `incremental-future-workspace-wiring` | workspace/flat 构建暂不落 cache、不 probe（WsPlan 缺 cache 目录 + gen 脚本需 `--no-incremental` 纪律）；单工程增量已落地 | [compiler/project.md](reference/src/toolchain/z42-toml.md#incremental-future-workspace-wiring) |
| `incremental-future-tsig-level-invalidation` | 文件级增量失效边取 token 保守粒度（引用文件任何变化即失效引用方，过近似只多编不错编）；「TSIG-equal 不失效」细化需 per-file TSIG 规范化 diff（剥全包自由函数泄漏段） | [compiler/project.md](reference/src/toolchain/z42-toml.md#incremental-future-tsig-level-invalidation) |
| ~~`self-hosting-future-single-vm-bootstrap-gap`~~ ✅ | 已解决 (2026-07-09) — fix-bootstrap-format-bump-deadlock：ci-bootstrap 版本差 gate + 两代自举(旧 VM 从 SDK bin/z42vm → gen1/gen2 → 新 VM;runtime/compile stdlib 分离),本地端到端验证。格式 bump CI 自动过、免手动 | [compiler/self-hosting.md](internals/src/compiler/self-hosting.md#self-hosting-future-single-vm-bootstrap-gap已解决-2026-07-09fix-bootstrap-format-bump-deadlock) |
| `packaging-future-mobile` | mobile（ios/android/wasm）包配置化；Phase 1 只覆盖 desktop SDK + runtime | [internals: packaging.md](internals/src/devinfra/packaging.md)（原 `design/toolchain/packaging.md` 的延后条目未随并书迁入，正文暂只在本行） |
| `packaging-future-selector` | `packages.toml` 自动发现（组件自报 `packages=[...]`，中央零编辑）；牺牲显式 include 的可见性，apphost 数量多时再评估 | [internals: packaging.md](internals/src/devinfra/packaging.md)（同上，正文暂只在本行） |
| `packaging-future-artifact-naming` | `packages.toml` 的 `[package.*].artifact` 模板未被 xtask 消费，包目录命名仍是既有硬编码拼接（与模板字面值不一致）；改成真读字段会变更包目录名（外部可见），Phase 1 不做 | [internals: packaging.md](internals/src/devinfra/packaging.md)（同上，正文暂只在本行） |
| `packaging-future-byte-identical-verification` | sdk/runtime 包树端到端逐字节一致验证被并行会话的 zpkg 格式升级（minor 22→0.23）环境问题阻塞；重构逻辑本身已通过单元+部分 e2e 独立验证 | [internals: packaging.md](internals/src/devinfra/packaging.md)（同上，正文暂只在本行） |
...
> User 速记清单里**在本文其余各节零覆盖**的条目；其余条目已分别落在 0.4.x 模块表（todo#1/#2/#4/#7/#8/#9/#10/#11/#12）与「横向工作流」里。无书页条目、无排期。
...
| ~~`generic-methods-future-classlevel-typeof`~~ ✅ **已落地**（fix-class-level-typeof，2026-09-25）| 类级 `typeof(T)` 在**实例语境**产出真实类型（载体 = 实例 `type_args`，与类级 `default(T)` 同源）。**用 builtin `__class_type_arg` 而非新 opcode** ⇒ 零 zbc 格式 bump、JIT 白送（`Instruction::Builtin` 按名派发）。正文见 [`language/generic-methods.md`](reference/src/language/generic-methods.md) | — |
| `classlevel-typeof-future-inherited-and-static` | 类级型参载体的两个空洞：①`class Derived : Box<int>` 的实例 `type_args` 为空 ⇒ 基类体内 `T` 取不到（`typeof` 与 `default` 都是）；②静态语境 `default(T)` **静默读第一个实参**的 `type_args`（`typeof` 侧已在绑定期拦住）。正解 = `ObjNew` 携带基链实参 + 按**声明类**寻址（声明类从帧的函数 owner 推导 ⇒ 零指令变更）；🔴 扁平下标在 `DerivedG<U> : Box<int>` 上无解（`U` 与 `T` 都想占下标 0）。需 fingerprint bump | 紧接 fix-class-level-typeof（User 2026-09-25 裁决拆两刀）|
| ~~`accept-boxed-struct-field-get`~~ ✅ **已落地**（2026-09-25）| 值 struct 经**擦除的返回位**流出泛型函数后读字段（`id(v).X`）从运行期崩 `FieldGet: expected object, got BoxedStruct` 改为正常取值。根因＝`field_get` 是**唯一**没有 `BoxedStruct` 接收者臂的指令（`vcall`/`is`/`as`/`struct_fget_prim`/数组元素整读/反射 `GetValue` 全都认）⇒ 补 interp + JIT 两条臂，复用反射的 `boxed_struct_field_get`（含布局三层对账）。**零格式变更、零编译器改动、零字节漂移**。⭐ 订正 `generic-constraints.md` 原记的「根因是泛型特化」——与特化无关（特化影响的是调用点**静态**类型）。golden `generics/erased_return_blob_field.z42`（四叶子 + 显式/推断型参 + 200k 热循环逼 OSR）| [book struct-value-semantics](internals/src/runtime/struct-value-semantics.md) 「`field_get` 接受装箱 struct」节 |
| ~~`unify-blob-return-abi`（PR 1/2：接口/型参边界）~~ ✅ **已落地**（2026-09-26）| 接口声明的返回位是**型参**（含 `Self`）时，调用点（擦除的泛型体 / 接口收者）把它当引用槽、不传 sret，而返回 blob struct 的具体实现有 sret ⇒ 编译期零诊断、运行期 `takes 3 physical argument(s), the call passes 2`（**坑点 ④a**：`where T : INumber` 对双字段 struct）与 `takes 2 …, passes 1`（新发现的第三副面孔：实例 `Self Copy()` 经接口收者）。修法沿 **#814 add-iface-return-bridge 的 A′**：桥接占裸名（无 sret、`__box_struct` 返回）、具体实现挑 `<m>$struct`；`IfaceBridgeSynth` 从硬编码「1 个 this + sret」**泛化到 N 形参 + 可静态**；**四处仍以裸 `RegKey` 作调用目标名的静态调用点改走 `CallKey()`**（#814 只改了实例路）。⭐ 订正 `static_abstract_operator.z42` 的抬头：它拿 `Money`（**单**字段、非 blob、2 对 2 巧合通过）当成「真正会炸的形态」⇒ 它一直是「写了但抵不到 bug」。零新指令、零 wire 格式变更；fingerprint 手动 bump 25（stdlib 中 `INumber` 实现者全是基元 wrapper = scalar ⇒ CI 守门对这一档是瞶的）| [internals missing-symbol](internals/src/runtime/missing-symbol.md) 「谁负责传 sret」节 |
| ~~`fix-crosspkg-static-sret`~~ ✅ **已落地**（2026-09-26）| 跳包调用返回 blob 值 struct 的**静态**方法/静态属性：消费方漏传 sret 槽 ⇒ 编译期零诊断、运行期 `takes N+1 physical argument(s), the call passes N`。`CallEmitter` 三条绕过 VCall 的捷径里，**两条实例路（devirt / DepIndex instance）一直正确拼 sret，唯独静态那条从一开始就没拼**。⭐ 四个月无人发现的原因：全仓**没有任何 golden 跳包调用过「返回 struct 的方法」**（`struct_cross_pkg` 只测构造与字段读），而 `z42.core` 里**一个多字段 struct 都没有**。新 fixture `cross-zpkg/single_field_struct_cross_pkg`。⭐ 调查教训：`--dump-ir`/`--dump-bound` **不加载 stdlib/依赖**，用它们看跳包调用点会得到假象（我据此错判成 loose VCall）| [internals missing-symbol](internals/src/runtime/missing-symbol.md) 「三条捷径里静态那条漏了 sret」节 |
| ~~`single-field-struct-value-semantics`（坑点 ⑤）~~ ✅ **已落地**（2026-09-26）| **单字段 struct 从引用语义改为值语义**（`S b = a; b.X = 50;` 不再改到 `a`）。闸门 `IsBlobStruct` 的 `FieldCount >= 2` 翻成 `>= 1`；🔴 **VM 侧有一份逐字镜像的闸门**（`exec_array.rs::try_struct_backed`）必须同步翻，只翻一侧 ⇒ `S[]` 元素全 `Null`；配套给 `extern` 桩新增 blob 返回的 sret 通道（全仓唯一这种形状是 `Std.GCHandle.Alloc`）+ Rust 侧 `GCHandle` 的读写改吃三种承载。User 裁决取 **⑤-blob**（而非归档原设想的单标量叶子塔缩）—— ⭐ `GCHandle` 自己的抬头文档写的就是值语义（拷贝共享同一 slot），blob 化是**兑现**而非破坏。golden `types/single_field_struct_value_semantics.z42`（10 组）| [reference structs.md](reference/src/language/structs.md) 「值语义与字段数无关」节 |
| `substitute-generic-call-return-type` | 把（显式写出的 / 推断出的）类型实参代换进**调用点的返回类型**，让 `id(v)` 的静态类型是 `Vec2` 而非裸 `T`（今天成员类型 `:<unknown>`、不受类型检查）。🔴 含一行真 bug：`MethodTypeArgSubst.ForExplicitTypeArgs` 拿 `TypeParamNames.**Length**`（＝ `TypeParser._parseTypeParams` 里 `new string[4]` 的**容量**、`Count` 另记）当型参个数 ⇒ **本地声明**的泛型方法/自由函数上显式 `<T>` 的签名代换**从未生效**；跳包导入的因 `_tpNames` 给的是精确长度数组反而生效。判别性探针：声明恰好 4 个型参时代换立即生效。⚠️ **不可单独修**——代换一生效，调用方按具体返回类型加 sret 而擦除的 callee 没有 ⇒ `takes 1 physical argument(s), the call passes 2`，即坑点 ④a。依赖链 ＝ **④a（泛型边界物理约定统一）→ 本条 → 单字段 struct 值语义**，需 `#814 add-iface-return-bridge` 桥接范式 | 与 ④a 同刀 |
| `reject-assign-to-erased-call-result` | `id(v).X = 5`（写进从擦除返回位流出的**临时**装箱 struct）今天运行期崩 `FieldSet: expected object`；写入必然被丢弃 ⇒ 应是**编译错误**（C# 同）。刻意不给 `field_set` 补 `BoxedStruct` 臂——那会把「必然无效的写」从响一声变成静默丢弃 | accept-boxed-struct-field-get 之后 |
| `cache-struct-reflect-layout` | 按类型名缓存 `struct_reflect::compute` 的 `ComputedLayout`（布局按类型不变 ⇒ 缓存天然安全）。accept-boxed-struct-field-get 把这条原本只服务反射的**冷**路径拉上了热路径：实测 500 万次 `id(v).X` = 2.29s、同规模普通对象字段读 0.78s ⇒ 每访问约 300ns 的重算。缓存挂 `VmContext` 还是 `TypeDesc` 是独立取舍 | 有热路径诉求时 |
| ~~`dispatch-tostring-in-native-stringify`~~ ✅ **已落地**（2026-09-25）| `Console.WriteLine(x)` / `Write(x)` 与**字符串拼接**现在派发用户 `ToString`；四条字符串化路（显式 / 插值 / 拼接 / `WriteLine`）对 class / record / 单字段 struct / 双字段 struct **给同一个答案**（无覆写 → 短类型名）。`fix-struct-tostring-paths` 只补了 `_isBlobStruct`（字段数 ≥ 2）那一支；本刀把 `builtin_println` 一族与 `exec_value::add` 接到早就在重入 VM 的 `obj_to_string`（+ 给它补装箱 struct 臂，复用 `resolve_vcall` 的判据）。⭐ **旧 golden 抬头记的「涉及 GC 根与可重入性 ⇒ 另开」前提是错的** —— 路②（`ToStr` 指令）一直在 `exec_function` 重入，重入不是新风险，只是没接线。零格式变更、零编译器改动；`compiler` 档墓铟时间持平（1m20s）| [reference structs.md](reference/src/language/structs.md) 「`ToString` 在所有字符串化路径上一致」 |
| `propagate-tostring-exception` | `ToString` 抛异常时字符串化产 `<exception: …>` 字符串（`obj_to_string` 的既有约定）。改成传播会让 `Console.WriteLine` / 拼接变成**可抛点** ⇒ 独立取舍 | 有诉求时 |
| `repl-value-display-contract` | REPL 回显对容器打 `List`（跟随 `ToString`）仍不如 `[1, 2, 3]` 有用。前置：先把 `z42.scripting/tests/` 下 **11 个显式声明的孤儿 fixture**（`driver.z42` + `expected_output.txt`，两套发现规则都不认领，`[tests] auto = false`）接进门禁 | REPL e2e 需真 tty |
