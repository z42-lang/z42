...
| ✅ 引用压 8B / `Value` 24→16B（B-radical 子目标，2026-08-11 提，**2026-08-15 落地 `unify-object-byte-layout` PR-3~5**）| 引用改平台指针大小（GcRef 16B→8B：**采路 A** 标记指针塞窄 generation 保非移动 GC；String `Arc<str>` 胖指针 16B→细指针 8B 长度进头；**FuncRef `Box<str>`→`Str` 8B**）→ `Value` 最大 payload 8B → enum 24B→16B（全 VM 密度/cache 33%）。**收益在密度非 native 交互**。全 VM 横切（JIT stride 24→`size_of::<Value>()`+`size_of::<Value>()==16` 编译期 pin）；不在 struct P3b。**残留 follow-up**：string 全 GC 化（`Value::Str`→`GcRef<StrHeader>`，需变长 GC 分配器） | [runtime/object-abi.md §2.1](design/runtime/object-abi.md) |
