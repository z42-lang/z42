...
| 逃逸分析栈分配 future（escape-stack-future-*）| ① JIT 侧 arena 落地（v1 JIT 忽略 flag）② 跨过程参数逃逸摘要（模块不动点，放宽 ctor 单函数摘要 + IsInstance）③ 标量替换（对象炸成寄存器，第二种 lowering）④ scope/回边级 arena 复位（v1 随帧退出释放，热循环内累积）| [add-escape-analysis-stack-alloc/design.md](spec/changes/add-escape-analysis-stack-alloc/design.md) "Deferred / Future Work" 段 |
