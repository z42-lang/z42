...
| ~~`Self` 返回位不做替换（self-return-type-substitution）~~ ✅ **已解决**（2026-09-07，change `self-return-type-substitution`）| 经**接口静态类型**调用返回 `Self` 的方法，结果类型现在是**该接口本身**（`IClone c; var x = c.Copy();` → `x : IClone`）——可靠上界，实现方必然实现该接口。具体类上调用不受影响（走实现方签名 → 具体类型）。⚠️ **形参位的 `Self` 仍未替换**（`bool Same(Self other)` 经接口调用）：逆变方向、无唯一安全上界，属独立议题 | [book: generic-constraints.md](book/src/language/generic-constraints.md) `Self` 类型 §实现模型 |
