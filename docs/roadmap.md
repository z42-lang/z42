...
| sealed 去虚化（add-sealed-devirt）| `impl-sealed-semantics` 拆出：receiver 静态类型是 sealed 类时把 `VCallInstr` 降级为直接 `CallInstr`，解锁 `IrInline` 内联 virtual 方法（净增价值=解锁内联，非派发提速——PIC 已覆盖派发）。地基已备（class/method `*_FLAG_SEALED` + 本地/跨包 `IsSealed`）；落点 `ExprEmitter._emitCall` + `EmitContext` 目标解析。拆分因目标解析（定义类+RegKey+abstract 跳过+本地/imported 约定）正确性敏感（错=静默 miscall），需独立测试矩阵 | [book sealed](book/src/language/sealed.md) "Deferred / Future Work" 段 |
