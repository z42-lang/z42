...
| `single-field-struct-value-semantics`（坑点 ⑤）| 单字段 struct 仍是**引用语义**（`S b = a; b.X = 50;` 会改到 `a`）。两条路待 User 裁决：**⑤-blob**（把 `IsBlobStruct` 的 `FieldCount < 2` 翻成 `< 1`；已实测爆炸半径 = 编译器自举**构建成功** + e2e **355/2**，两条失败全是 sret 失配）vs **⑤-scalar**（归档设计原意的**单标量叶子塔缩**，`GCHandle` 保持裸 `long` ⇒ native 边界零改动）。🔴 取 ⑤-blob 则**必须自带 ⑤-b 的判据**：`GCHandle.AllocStrong` 返回 blob 后，跳包调用点发的是 **loose VCall**（实测 IR `vcall %27.AllocStrong(%26)`、无 sret 槽），而它**不实现任何接口** ⇒ unify-blob-return-abi 的接口满足性打标够不到，需「导出的、返回 blob struct 的方法」这个独立判据 | 待 D3 裁决 |
