...
| sealed 去虚化 v1 边界外（add-sealed-devirt-future）| **v1 已落地**（`add-sealed-devirt`：本地非泛型 sealed 类 receiver → `VCall` 降级直接 `Call` 解锁 `IrInline`；`Opt.Devirt` bit11）。**待覆盖**：① imported sealed 类（跨包目标名/RegKey 约定）② 泛型 sealed 类（`$N` mangle）③ `sealed override`（receiver 基类型，需精确类型/单实现分析）。这些情形 v1 回落 VCall（仍正确、只是不内联） | [book sealed](book/src/language/sealed.md) 「去虚化」+「Deferred」段 |
