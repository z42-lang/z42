...
### 🔴 约定只在《阅读说明》交代一次，各章不重复

**通用约定（代码从哪来、片段与完整文件的关系、终端画面怎么读、`>` 引用块与 ⚠️ 标记的含义）
一律写在 `getting-started/how-to-read.md`**，各章**不得**再写「本章代码在 examples/… 」
这类章首说明——重复一遍等于多一处会漂的真相。后续新增的通用阅读约定也往那一页加。

### 各章要做的只有一件：让读者知道眼前这段代码是哪个文件

两种写法，按章节性质选：

**写法甲：散文点名（手把手的章节用）。** 起步部分那种一步步带着建文件的章节，直接在正文里
说「新建一个名为 `hello.z42` 的文件」「把它挪到 `src/Greeting.z42`」，再**整文件** include。
读者要建的文件、完整内容、怎么跑，一条线下来全有了，不需要额外标注。**章尾给出本章示例目录
的链接。**

**写法乙：每段 z42 代码上方标出完整仓库路径（片段式的章节用）。** 语言基础往后，一章要展示
十几个互不相关的片段，不可能每个都手把手：
...
- **写完整的仓库相对路径**，不要只写章节目录下的尾巴。标注是该段代码出处的**唯一**说明，
  必须自成一体——读者可能从搜索结果直接落到这一页。
- `console` 块**不标**——它自己的 `$ z42 run <文件>` 行已经说明了跑的是谁。
- 同一文件的两段紧邻时，**第二段不要重复标注**，换成一句引导语（「在 `Main` 里用它们：」），
  既去掉重复又把两段的关系说清楚。
