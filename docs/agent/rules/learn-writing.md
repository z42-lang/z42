...
> [doc-system.md](doc-system.md) 第二节「学习手册」与决策 D10；知识库的写法见 [book-writing.md](book-writing.md)。
...
| | 学习手册 `docs/learn/` | 知识库 `docs/book/` |
|---|---|---|
| 回答 | 怎么用 | 是什么、为什么 |
| 组织 | 按学习顺序，后一章只依赖前面讲过的内容 | 按主题，规则完整 |
| 读者 | 使用 z42 的开发者 | 用户 + 维护者 + 大模型 |
| 代码 | 全部来自 `examples/`，逐条运行校验 | 片段，说明机制 |

手册讲到某条规则时只讲够用的部分，完整规则**链接**知识库对应页，不复制。
