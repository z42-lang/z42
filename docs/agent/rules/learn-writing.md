...
**写法乙：代码块第一行写出处注释（片段式的章节用）。** 语言基础往后，一章要展示十几个
互不相关的片段，不可能每个都手把手：
...
- **写完整的仓库相对路径**，不要只写章节目录下的尾巴。它是该段代码出处的**唯一**说明，
  必须自成一体——读者可能从搜索结果直接落到这一页。
- **门禁校验这行注释与 `{{#include}}` 指向同一个文件**（B6）。这是它比页面上手写的
  加粗标注强的地方：标注没人管，改了 include 忘了改标注不会判红，注释会。
- 放在**块内**而不是块上方，是为了复制代码时把出处一起带走。
- `console` 块**不写**——它自己的 `$ z42 run <文件>` 行已经说明了跑的是谁；写了会判红。
- 同一文件的两段紧邻时，**第二段照写注释**（它是块内的一部分，不构成视觉重复），
  但正文里用一句引导语把两段的关系说清楚（「在 `Main` 里用它们：」）。
