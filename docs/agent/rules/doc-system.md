...
**`reference` 的正文不得链 `internals`。** 一旦正文反向链，用户就会被带进实现细节——受众混淆
会从头开始。`internals` 可以链任何一边；`learn` 链 `reference` 查规则，**正文同样不链 `internals`**。

一句话记法：**面向用户的两本书（learn / reference）的正文里，不出现任何指向 `internals/` 的链接。**
需要交代「实现细节在别处」时，用**不带链接的散文**（「具体降解过程属实现细节，本手册不展开」），或者不提。

#### 唯一的例外：各书首页的「去哪」导航表（2026-09-22，User 裁决）

**每本书的首页**（`src/README.md`）的导航表**可以列出另外两本书**，含 `internals`。

理由：三本书同站发布却互不指路，等于把「我要改 z42 本身」的读者堵在参考手册首页——
他得先猜到还有第三本书、再自己拼 URL。而首页导航表本来就在做「指路去别的书」这件事
（reference 首页早已链 learn）。

边界很窄，好执行：
- **只有首页的导航表**，不是首页正文的任意段落，更不是任何参考页 / 章节页。
- 一行链接，不复述 internals 的内容。

原规则要保的东西没动：读者在**查语法、查 API 的页面上**永远不会被带去实现细节。
...
3. **不写历史**：文档只描述**当前状态**。三类一律剥离——
   ...
   > **但「还没做什么」要写。** ToDo、已知缺口、后续迭代机会**属于当前状态的一部分**——
   > 读者要靠它判断这块能不能依赖。禁的是**回头看**（做过什么、什么时候做的、取代了什么），
   > 不是**向前看**。
   >
   > | 写 | 不写 |
   > |---|---|
   > | 「弱引用目前不保证失效时机」 | 「弱引用在 0.5.2 修好了」 |
   > | 「`[Timeout]` 当前不生效，别指望它兜底」 | 「`[Timeout]` 原本计划在 R4 落地」 |
   > | 「下一步可以按 card 粒度收敛 remembered set」 | 「曾经用过全堆扫描，后来改了」 |
   >
   > 落点：书页页头的**可选**「待办」行（见 [book-writing.md](book-writing.md)）或正文的
   > 「不支持」/「已知边界」节；**长期项目计划仍归 `docs/roadmap.md`**，书页只写与本页主题绑定的那几条。
