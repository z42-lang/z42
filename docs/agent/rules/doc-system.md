...
├── learn/       【学习手册】mdBook——面向用户的教程："怎么用"，按学习顺序；代码全部 include 自仓库根 examples/
...
附：**学习手册**（`docs/learn/`）——面向用户的教程，独立于以上四类。与知识库分工：**手册讲「怎么用」**
（按学习顺序、从安装到发布），**知识库讲「是什么 / 为什么」**（按主题、规则完整）。手册只**链接**知识库的
机制与规则，不复制；手册里的 z42 代码与终端输出全部 include 自仓库根 `examples/`，由 `xtask test examples`
逐条运行校验。写作规范见 [learn-writing.md](learn-writing.md)。
...
| **D10** | **学习手册独立成书**（`docs/learn/`，同站发布于 `/z42/learn/`），不并入知识库；`examples/` 只放手册配套工程、不再是特性演示库（2026-09-15 User 裁决） | 教程与知识库的读者、组织顺序、页型完全不同，混在一本里两边都难检索；示例绑定手册并逐条运行校验，才不会像旧 examples 那样无人编译、悄悄腐烂 |
...
### 5.1 复杂实现逻辑流程必须写入 book（2026-07-07 确立）
...
**写法**：机制页用伪代码 / mermaid 呈现流程 + 一段"为什么"（决策 / 权衡 / 坑），使新接手者不必
读大量源码即可理解（对齐 [CLAUDE.md 实现原理文档规则](../../../.claude/CLAUDE.md) + [book-writing.md](book-writing.md)）。
源码只留"怎么做"，book 承载"为什么 + 整体流程"。

**不确定是否算"复杂"、该不该写、写哪一页 → 停下问 User，别默默略过。** 漏写复杂逻辑 =
知识只活在作者脑子里，与本体系"单一权威、不过时"的目标相悖。
...
2. **不保留历史对照等过时内容**：文档只描述**当前状态**。三类都要剥离——
   - 考古注记：`（2026-05-20 add-threading-stdlib）` / `（2026-08-29 起）` 这类来源 / 起始日期标注，演进史归 git blame / spec archive
   - 迭代过程 / 状态叙述：「落实于 B 阶段」「迁移状态（✅ 完成）」「取代旧方案」「后续阶段落地」
     这类"何时 / 怎么加进来的、原来是什么、之后要做什么"的过程话——文档只写**终态结构**，
     不叙述抵达终态的过程；change 溯源归 README「关联文档」段的 change 名（+ git / spec archive）
...
**例外：学习手册（`docs/learn/`）中文先写**（User 2026-09-15），内容与示例稳定后再出英文版（另一套 SUMMARY，共用 `examples/`）。
