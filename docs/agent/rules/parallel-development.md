...
| 时间 | 事件 |
|---|---|
| 01:51 | #577 并入 main 最新改动、跑完整 GREEN → 全绿 |
| 04:49 | **#580 落地 main**，新增 `tests/dirunit/` fixture（正是照出误报的那个） |
| 06:36 | #577 被合并 —— 中间**没有任何东西重新验证过** |

⇒ main 三平台 `test-host` 全挂。§3 被遵守了（并入 + 重跑），但做在**合并前几小时**；
窗口里落地的 PR 让那次 GREEN 的结论过期。修复见 #586（E0606 对 dev-target 父包误报）。

**事故二（本地 GREEN 被热产物掩盖）**：同一天，`xtask test` 在本地连续多轮全绿，而 CI 恒红 ——
夹具的构建产物是**热**的（缓存命中就不重编），那条编译错误只在**冷构建**出现。
...
> 两条事故同一个形状：**绿灯的前提变了，绿灯却不会自己失效**。前提是 base sha，或是产物状态。

### §3.2 改「跑测试的工具」时，它依赖的源码修复必须**先**进 main（2026-09-13）
...
现场（#611 / #613，2026-09-13）：bench 路径转发 z42b 时顺手补了 `core_bench.z42` 缺的
`using Std.Collections;`，两者同 PR ⇒ `bench-regression` 红。把 `using` 拆成 #613 先合，
#611 rebase 后即绿。

> 同族提醒：#602（test 路径转发）也补过两处缺失的 `using`，只是那条路径没有「用新 runner 跑
> 旧树」的步骤，侥幸没红。**别把「这次没红」当成可以合并提交的依据。**
...
有一类冲突 git 也测不出来：两个 PR 各自从一张登记表里挑了同一个**没人用的编号**（诊断码号最典型，
2026-09 连撞三次：E0474 / E0477 / E0481）。它不是语义耦合，也不总是文本冲突——各自都合法，
**合起来才错**。
