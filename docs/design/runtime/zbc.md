...
| 1.37 | 2026-08-23 | [attribute-handler-registry](../../spec/changes/attribute-handler-registry/)（PR5） | **`[Deprecated]` 持久化**（D2）。**方法级**：`SIGS method_flags:u8` bit3=deprecated（`METHOD_FLAG_DEPRECATED`，`method_flags` 是既有字节、bit3 空闲），置位时 SIGS 条目尾部追加 `deprecation_msg:u32`（串池 idx，gated）。**类/字段级**：**TYPE 段体尾部 size-gated 弃用表**（`class_flags` 已满、bit6=delegate 不可用；per-field 非-gated 字节会破两代自举，故用尾表）——所有 class 记录后，若本模块有任何类/字段弃用则追加 `classDepCount:u16 + (classIdx:u16, msgIdx:u32)× + fieldDepCount:u16 + (classIdx:u16, isStatic:u8, fieldIdx:u16, msgIdx:u32)×`。**两代自举安全**：未弃用模块（含全 stdlib）无尾表 → TYPE 段体到 classes 末即止、与旧格式逐字节一致；reader 靠「cursor < 段末」判尾表是否存在（gen1 读 gen0 无尾表→不读→无脱同步）。**zpkg MODS 段面不变**（尾表在 TYPE 段体内）。VM 侧 class/method/field 弃用 **read-and-discard**（弃用是编译期 use-site 告警，无运行时强制）。耦合 zpkg 0.42。Pre-1.37 zbc 不可读 |
