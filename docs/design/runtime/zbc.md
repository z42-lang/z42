...
| 1.37 | 2026-08-23 | [attribute-handler-registry](../../spec/changes/attribute-handler-registry/)（PR5） | **`[Deprecated]` 持久化**（D2）。① **SIGS `method_flags:u8` 新增 bit3=deprecated**（`METHOD_FLAG_DEPRECATED`），置位时 SIGS 条目尾部追加 `deprecation_msg:u32`（串池 idx，gated）。② **TYPE `class_flags:u8` 新增 bit6=deprecated**，置位时紧随类可见性字节追加 `deprecation_msg:u32`（gated）。③ **每条 TYPE 字段记录（实例+静态）新增 `field_flags:u8`**（bit0=deprecated；此前字段仅 visibility、无 flags 字节，**每字段恒 +1 字节**），置位时追加 `deprecation_msg:u32`（gated）。VM 侧 class/method 消息 **read-and-discard**（弃用是编译期告警，无运行时强制）；`FieldDesc` 存 deprecated/msg 作休眠反射元数据。耦合 zpkg 0.42。Pre-1.37 zbc 不可读 |
