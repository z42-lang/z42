...
- **当前版本**：`major=0, minor=43`（详见下方 Minor changelog）
...
| 0.44 | 2026-09-13 | [encode-ctorless-objnew](../../spec/changes/encode-ctorless-objnew/) | 耦合 inner zbc 1.39（`ObjNew` 尾部加 `ctor_known:u8` —— 编译期整包装配后置的**正向**位，供运行期区分「构造器缺失」与「本来就无构造器」，见 zbc changelog）。zpkg outer 段面不变。bump 触发 ci-bootstrap 版本差 gate → 两代自举吸收 |
