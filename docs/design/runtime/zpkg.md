...
| 0.42 | 2026-08-23 | [attribute-handler-registry](../../spec/changes/attribute-handler-registry/)（PR5） | 耦合 inner zbc 1.37（`[Deprecated]` 持久化：SIGS method_flags bit3 + gated msg / TYPE class_flags bit6 + gated msg / 每字段新增 field_flags 字节 + gated msg，见 zbc changelog）。zpkg outer 段面不变。bump 触发 ci-bootstrap 版本差 gate → 两代自举吸收 |
