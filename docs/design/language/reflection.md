...
- **剩余**：泛型变体 / 传递接口 延后。**大小写不敏感 GetInterface（C# `ignoreCase` 重载）—— 不做（2026-07-24 定）**：z42 是大小写敏感语言，类型/接口名精确匹配（`GetInterface` 已 `t.Name == name` 严格比较，反射全链无任何 case-insensitive 逻辑）。C# 的 `ignoreCase` 重载是继承自大小写不敏感语言（VB 等）的遗留，与 z42 语言身份不符、且引入歧义（仅大小写不同的两接口），故明确不引入。~~handle-less 装箱语义（`object.IsAssignableFrom(int)`）~~ —— 已落地 2026-07-24（add-reflection-object-assignable，纯 runtime）：`typeof(object)` 是 handle-less Type（名 `"object"`），此前 `IsAssignableFrom` 落 FullName 相等 → 对值类型/用户类/接口/数组恒 false；现特判 `this` 名为 `"object"`/`"Std.Object"` → 对任意非 null `c` 返 true（值类型装箱 + 所有引用类型/接口/数组皆派生自 object，镜像 C#）。不过度放宽——`int.IsAssignableFrom(long)` 仍 false。
...
- **剩余（Deferred，真实卡点）**：**泛型外层** `Outer<T>.Inner` —— 卡两处：① parser 类型位置不接受 `Generic<Args>.Nested` 语法（`Box<int>` 后不消费 `.Tag`）；② 内层引用外层 `T` 的嵌套类型需 generic instantiation 做 T 替换（0.5.x L3-R，与 `reflection-future-method-invoke` 同前置）。已「试做」验证到此边界、如实标出，非静默延后。~~嵌套类型的 base 为**另一嵌套类型**（`class Derived : Base` 兄弟 / `class D : Outer.Base` 限定 / 实现兄弟嵌套接口）~~ —— ✅ 已落地 2026-07-25（nested-types-followup）：`NestedFlatten` 在 AST 上把嵌套 base/接口引用限定到 `+` 全名（stub 判别 + IrGen base_name 发射都读 AST，一处根治），继承字段 + 虚派发 + 上转型 + 嵌套接口 impl/GetInterfaces 全通。跨包**限定名**引用嵌套（`geo.Shape.Corner`）当前只解析包内 `Outer.Inner`；嵌套 partial 仍不支持（E0435 保留，design D9）。
