...
| `src/OptSet.z42` | **可独立开关的具名优化位集**（`Opt` static class；add-compiler-inlining）：`ConstFold=1/CopyProp=2/Dce=4/Inline=8/Cse=16/Licm=32/StackAlloc=64/All=127` + `Has`/`ByName`/`ProfileDefault(isRelease)`（debug=None/-O0、release=All）/`Resolve`（CLI>toml>profile）|
| `src/IrEscapeAnalysis.z42` | **逃逸分析栈上分配 pass**（`Opt.StackAlloc`，入 All；add-escape-analysis-stack-alloc）：CFG-free 流不敏感 may-escape 过近似——`_computeEscapedRegs`（Pass A 角色感知逃逸汇点规则表 `_markEscaping` 入种子 + Pass B copy 传递闭包）+ ctor this-escape 单函数摘要 `_ctorLeaksThis`（对象合格前提）。不逃逸 + 单赋值的 `ObjNew`/`ArrayNew`/`ArrayNewLit` → 置 `StackAlloc=true`（interp 帧 arena 分配、绕过 GC）。规则表镜像 `AddReads`、未知指令保守判逃逸。见 book escape-analysis-stack-alloc |
| `src/IrLicm.z42` | **循环不变量外提 pass**（`Opt.Licm`，入 All）：CFG + 迭代支配 `_computeDom` + 回边→自然循环（并同 header 多回边体）+ 保守 pre-header（现成 `br h` 前驱）+ 不变量（IsPure + 单赋值 dst + 操作数不在 **header 支配域**内定义）+ 外提。**跳过有异常表的函数**（`ExcCount>0`——CFG 不含异常隐式边）。见 book optimization-pipeline |
| `src/IrOptPipeline.z42` | **编译期 IR 优化管线**（IrGen.Generate 末尾）：`Run(m, optSet)` 按 `Opt.Has` 门控每 pass（`None`→整体跳过=-O0）。先模块级 inline（`IrInline.Run`，靠前产更多下游机会），再逐函数 const-fold → **licm**（`IrLicm.Run` 循环不变量外提）→ **cse**（`_passCse` 块内 value-number 去重）→ copy-prop（producer-retarget + **use-site 级联** `_passCopyPropUse`，靠 `ReplaceReads`）→ temp-DCE（各自重算 reads/defs → 单独开也正确，design D2）。interp-first，见 book optimization-pipeline |
