...
| `src/OptSet.z42` | **可独立开关的具名优化位集**（`Opt` static class；add-compiler-inlining）：`ConstFold=1/CopyProp=2/Dce=4/Inline=8/Cse=16/Licm=32（opt-in，不入 All）/All=31` + `Has`/`ByName`/`ProfileDefault(isRelease)`（debug=None/-O0、release=All）/`Resolve`（CLI>toml>profile）|
| `src/IrLicm.z42` | **循环不变量外提 pass**（`Opt.Licm`）：CFG + 迭代支配 `_computeDom` + 回边→自然循环 `_tryLoop` + 保守 pre-header 判定（现成 `br h` 前驱）+ 不变量（IsPure + 操作数全循环外）+ 外提到 pre-header。见 book optimization-pipeline |
