...
| `src/IrOptPipeline.z42` | **编译期 IR 优化管线**（IrGen.Generate 末尾）：逐函数建读/写计数（含参数寄存器 live-out）→ 跑 pass。pass 1 const-fold（两 int 常量运算/比较→Const，`TryConstFold` 规则表可扩展）；pass 2 copy-prop（`t=expr; copy local,t`→`local=expr`，砍 SSA-lite 冗余 Copy）；pass 3 temp-DCE（删纯+Dst零读的死指令）。interp-first，见 book optimization-pipeline |
