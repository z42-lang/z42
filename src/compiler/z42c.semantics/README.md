...
| `src/IrOptPipeline.z42` | **编译期 IR 优化管线**（IrGen.Generate 末尾）：逐函数建读/写计数（含参数寄存器 live-out）→ 跑 pass。pass 1 copy-prop（`t=expr; copy local,t`→`local=expr`，砍 SSA-lite 冗余 Copy）；pass 2 temp-DCE（删纯+Dst零读的死指令）。const-fold 后续。interp-first，见 book optimization-pipeline |
