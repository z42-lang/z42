...
//!     it in `<sdk>/programs/z42i/` (beside the interactive apphost payload); dev drops
//!     it in the cargo target dir beside `z42vm`. Found by *this* repl-specific probe
//!     (`candidates()`), never `ext::native_search_paths()`. Keeping it out of the shared
//!     `bin/` stops the eager ext scanner from warning `ignoring unknown lib repl` on
//!     every VM run (isolate-repl-cdylib). wasm/mobile never load it (plain fallback).
//!
//! # Boundary invariant
//!
...
mod native {
    ...
    ///      apphost's payload, deliberately isolated from the shared `bin/` so the
    ///      generic ext scanner (`ext::native_search_paths`, which scans the running
    ///      binary's own dir) never sees it and never warns `ignoring unknown lib
    ///      repl`. Derived from the running `<sdk>/bin/<app>`. (isolate-repl-cdylib)
        ...
        if let Ok(exe) = std::env::current_exe() {
            if let Some(dir) = exe.parent() {
                ...
                if let Some(sdk_root) = dir.parent() {
                    out.push(sdk_root.join("programs").join("z42i").join(&file));
                }
            }
        }
        out
