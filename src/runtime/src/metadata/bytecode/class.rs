...
/// complete-generic-instantiation：这个成员**纯由类型的声明形状导出**（字段序列 + record 性），
/// 因此**持有导出元数据的消费方能一字不差地重建它**。
///
/// 置位的：record 主构造器、record 的 `Equals`/`GetHashCode`/`ToString`、blob struct 的
/// `Equals$1`、继承来的 / 默认构造器。
///
/// ⚠️ **不**置位的（判据比「编译器生成了这个函数对象」窄得多，别按后者理解）：
///   · `[Native]` / `abstract` 桩 —— **声明是用户写的**，消费方重建不出来；
///   · 自动属性的 getter/setter —— **属性是用户声明的**；
///   · `$cctor` —— 体是**用户写的静态字段初始化式**。把它当「可重建」会让消费方重建后
///     **静态初始化整个丢掉**；不置位 ⇒ 带静态初始化的类型自动被判据拒掉，正是要的行为。
///
/// 用途：消费方据此判断「一个**导入的**泛型定义能否在本包按实例化布局重新发射」——
/// 要求**全部**导出成员都带这个位，于是重建不会顶替掉任何用户代码。按方法名猜是不行的：
/// 用户手写的 `ToString()` 与合成版**同名同签名**，猜会静默丢掉用户实现。
///
/// **零格式 bump**：`method_flags: u8` 的 bit4–7 本就空闲，旧读端按 u8 读、忽略不认的位；
/// 旧写端打 0 ⇒ 消费方判否 ⇒ 退回原行为。两个方向都优雅降级。
pub const METHOD_FLAG_SHAPE_DERIVED: u8 = 1 << 4;
