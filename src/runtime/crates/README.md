...
2. **驱动 / 运行 VM 侧**（`z42-host` / `z42-hostrun`）——在 VM 之上：`z42-host` link runtime crate
   在**进程内**嵌入执行；`z42-hostrun` 不 link，去磁盘找已部署的 `z42vm` 当**子进程** exec。
...
| `z42-hostrun/` | `z42-hostrun` | 之上 | 定位已部署 `z42vm` + libs 并**子进程** exec app 的 zpkg（apphost run 路径）；纯 std、零依赖、**不 link VM** |
...
```
        独立 native 扩展 (.so)        嵌入式应用 (host)
              ...
              z42-rs ──→ z42-abi ◀──── z42-host ──→ z42 (runtime crate)
                │            ▲              │              │
                ▼            │              ▼              ▼
            z42-macros ──────┘         z42-hostrun     z42-abi
             (展开引用 abi 类型)      (exec z42vm 子进程，不 link)
...
- apphost run 路径：change `add-apphost` / `simplify-apphost-direct-run`（`docs/spec/archive/`）；`z42-hostrun` 从 launcher lib 抽出的背景见其 crate 头注释（apphost-to-workload, 2026-06-18）

## 状态

`z42-abi` / `z42-macros` / `z42-host` / `z42-hostrun` / `z42-compression` / `z42-repl` 均为在用产物；
