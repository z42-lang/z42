...
| 缩进感知键位 + 回车整块判定 | `Std.Repl.ReplEditing.KeyEdit(key,line,pos)`（`ReplEditing.z42`；退格→`dedent`、Tab→`insert:<空格>` grid-snap、Enter→`accept`（写完提交）｜`newline:<缩进>`（没写完续行））+ 自由函数 `replKeyEdit`；决策全在 z42，Rust `parse_action` 只译成 redo-免疫 `Cmd`（见 z42vm `corelib/repl_editing.rs`）|
...
```
键位策略的纯函数覆盖在 z42vm Rust 单测 `corelib/repl_editing_tests.rs`（`cargo test --lib`）；
...
- 设计/机制：[`docs/design/toolchain/repl.md`](../../../docs/design/toolchain/repl.md)；
  键位适配壳（policy-free、动作串范式）见 z42vm `corelib/repl_editing.rs` 头注
- 引入/演进：change `add-z42-repl`（REPL MVP）/ `add-repl-indent-editing` / `add-repl-tab-grid-snap` /
  `add-repl-multiline-editing`（键位与多行编辑）；`split-z42-repl`（本包从 z42.scripting 拆出）

## 核心文件
| 文件 | 职责 |
|------|------|
| `Repl.z42` | `Std.Repl.Repl`：rustyline 行编辑原生绑定（`ReadLine` / `SetCompleter` / `SetKeyEditor`）|
| `ReplEditing.z42` | `Std.Repl.ReplEditing.KeyEdit`：键位策略（退格 `dedent` / Tab `insert:` grid-snap / Enter `accept`｜`newline:<缩进>` 整块多行判定，复用 `Completeness`）+ 自由函数 `replKeyEdit` 回调 |
