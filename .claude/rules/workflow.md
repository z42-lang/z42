...
6. **提供续推口令（必做，2026-08-11 新增）**：**任何任务 / 需求合并或提交落地后，若还有后续任务（同一程序的
   ...
   1. **把后续任务写进 memory**（`memory/<slug>.md`）——**关键：需求说明与前因后果必须写透**，让清理上下文后
      的新会话不丢背景、清楚「**为什么做、要做什么、边界在哪**」，而非只留一个干巴巴的任务名。至少覆盖：
      - **需求 / 目标**：这个后续任务解决什么问题、User 的原始诉求是什么（原话意图）。
      - **前因后果 / 背景**：它从哪来（属哪个大程序 / 哪个 Deferred）、**已做过什么**（已合并 commit/PR + 各自
        解决了什么）、**已裁决的关键决策及其理由**（避免新会话推翻或重问已定的事）。
      - **关键机制入口**：相关文件 / 函数 / 数据结构的 file:line 与作用（**勿让下个会话重新探索已知事实**）。
      - **未决设计分叉**：须 User 裁决的点 + 已列的备选方案 + 推荐。
      - **恢复环境**：worktree 路径 / 种子 / 构建·验证配方 / 已知教训（踩过的坑）。
      - **流程**：是否规范先行、拆分建议、下一步第一个动作。

      判据：**一个对本程序零记忆的新会话，只读这一份 memory（+它链接的），就能理解全貌并正确接手，不需要回看
      已清理的对话**。写完自检这一条。
   2. **定义口令**：一句简短、无歧义的中文触发语（如「推进 struct P3」）。在该 memory 文件 frontmatter
      `description` 和 `MEMORY.md` 置顶指针行显式标注「🔑 口令『…』→ 读本文件续推」，使新会话**自动加载
      `MEMORY.md`** 时即可看到并据以恢复（承接[会话恢复协议](#会话恢复协议)）。
   3. **回复 User 时明确给出口令**：告知 clear 后说该口令即可续推，并附一句当前落地状态。

   > 目的：把「跨会话续推所需的全部上下文」沉淀到 memory 而非留在对话里，让长程序（多 PR 阶梯）能安全地按
   > 任务边界切分会话、显著降低 token 成本。**没有后续任务时**（程序整体完成）无需口令，正常收尾即可。
